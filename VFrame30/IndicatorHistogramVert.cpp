		...
		for (IndicatorSetpoint& sp : result)
		{
			...
			if (comparatorValue.has_value() == true)
			{
				...
				case SetpointSource::Custom:
					...
					{
						overrideColor = (sp.customSetpointData->colorSource() == E::IndicatorColorSource::StaticColorFromStruct) ? std::optional<QRgb>(sp.customSetpointData->color().rgb()) : std::optional<QRgb>();
					}
					break;
				}
