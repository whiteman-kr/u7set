...
namespace VFrame30
{
	...
	bool SchemaItemVduLine::SaveData(Proto::Envelope* message) const
	{
		...
		// --
		//
		auto* lineMessage = message->mutable_schemaitem()->mutable_vduline();
	...
	bool SchemaItemVduLine::LoadData(const Proto::Envelope& message)
	{
		...
		// --
		if (message.schemaitem().has_vduline() == false)
		{
			assert(message.schemaitem().has_vduline());
			return false;
		}

		const auto& lineMessage = message.schemaitem().vduline();
