...
void DbControllerSignalTests::getSpecificSignalsTest()
{
	...
	for (Signal currentSignal : result)
	{
		...
		QVERIFY2(query.value("lowengeneeringunits").toInt() == currentSignal.lowEngineeringUnits(), qPrintable(QString("Error: value lowengeneeringunits is not match (Database: %1, DbControllerFunction: %2)").arg(query.value("lowengeneeringunits").toInt()).arg(currentSignal.lowEngineeringUnits())));
		QVERIFY2(query.value("highengeneeringunits").toInt() == currentSignal.highEngineeringUnits(), qPrintable(QString("Error: value highengeneeringunits is not match (Database: %1, DbControllerFunction: %2)").arg(query.value("highengeneeringunits").toInt()).arg(currentSignal.highEngineeringUnits())));
