...
public:
	...
	int						append(TestSignal* pSignal);

	TestSignal*				signal(const QString& appSignalID);
	TestSignal*				signal(const Hash& hash) const;
	TestSignal*				signal(int index) const;

	AppSignalState			state(const Hash& hash);
