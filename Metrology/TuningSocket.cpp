...
void TuningSocket::requestTuningSourcesInfo()
{
	...
	ServerType serverType = static_cast<ServerType>(selectedServerIndex());
	if (ERR_SERVER_TYPE(serverType) == true)
	{
		return;
	}

	QString equipmentID = theOptions.socket().client(SocketType::TuningSrv).equipmentID(serverType);
...
void TuningSocket::requestReadTuningSignals()
{
	...
	for (int i = 0; i < TUNING_SOCKET_MAX_READ_SIGNAL; i++)
	{
		...
		const Metrology::Signal& signal = theSignalBase.tuning().signalBase().signal(i + startIndex);
		if (signal.param().isValid() == false)
		{
			continue;
		}

		m_readTuningSignals.mutable_signalhash()->AddAlreadyReserved(signal.param().hash());
...
void TuningSocket::replyReadTuningSignals(const char* replyData, quint32 replyDataSize)
{
	...
	{
		const Network::TuningSignalState& state = m_readTuningSignalsReply.tuningsignalstate(i);
		theSignalBase.tuning().signalBase().setState(state);

		//qDebug() << theSignalBase.signalPtr(state.signalhash())->param().customAppSignalID() << " - " << state.valid();
	}
...
{
	HostAddressPort addr1 = theOptions.socket().client(SocketType::TuningSrv).address(ServerType::Primary);
	HostAddressPort addr2 = theOptions.socket().client(SocketType::TuningSrv).address(ServerType::Reserve);
