#ifndef MEASUREVIEWHEADER_H
#define MEASUREVIEWHEADER_H

#include <QColor>

#include "MeasureBase.h"

// ==============================================================================================

#define					MVC_CMN_HIDE				false
#define					MVC_CMN_SHOW				true

#define					MVC_CMN_DISABLE_DUPLICATE	false
#define					MVC_CMN_ENABLE_DUPLICATE	true

// ==============================================================================================

#define					MVC_CMN_COLOR_LIGHT_BLUE	QColor(0xE0, 0xFF, 0xE0)

// ==============================================================================================

namespace Measure
{
	class HeaderColumn
	{
	public:

		HeaderColumn();
		HeaderColumn(const HeaderColumn& from);
		HeaderColumn(const QString& uniqueTitle, const QString& title, int width, bool visible, int alignment, bool duplicate);
		virtual ~HeaderColumn();

	public:

		int					index() const { return m_index; }
		void				setIndex(int index) { m_index = index; }

		QString				uniqueTitle() const { return m_uniqueTitle; }
		void				setUniqueTitle(const QString& title) { m_uniqueTitle = title; }

		QString				title() const;
		void				setTitle(const QString& title) { m_title = title; }

		int					width() const { return m_width; }
		void				setWidth(int width) { m_width = width; }

		bool				enableVisible() const { return m_enableVisible; }
		void				setVisible(bool enable) { m_enableVisible = enable; }

		int					alignment() const { return m_alignment; }

		bool				enableDuplicate() const { return m_enableDuplicate; }

	private:

		int					m_index = -1;

		QString				m_uniqueTitle;
		QString				m_title;
		int					m_width = 100;
		bool				m_enableVisible = MVC_CMN_SHOW;

		int					m_alignment = Qt::AlignLeft;

		bool				m_enableDuplicate = MVC_CMN_ENABLE_DUPLICATE;
	};

	// ==============================================================================================

	const int				MaxColumnCount	= 64;

	// ==============================================================================================

	class ViewHeader : public QObject
	{
		Q_OBJECT

	public:

		explicit ViewHeader(QObject* parent = nullptr);
		virtual ~ViewHeader();

	public:

		void				setMeasureType(Measure::Type measureType);

		void				init(Measure::Type measureType);

		int					count() const;
		HeaderColumn*		column(int index) const;

		void				updateColumnState();

		void				setColumnTitle(int column, const QString& title);
		void				setColumnVisible(int column, bool visible);

	private:

		static				HeaderColumn m_column[Measure::TYPE_COUNT][MaxColumnCount];

		Measure::Type		m_measureType = Measure::Type::NoMeasureType;

		int					m_columnCount[Measure::TYPE_COUNT];
	};
}

// ==============================================================================================
// Measure::Type::Linearity
//
const int				MVC_CMN_L_INDEX				= 0,
						MVC_CMN_L_MODULE_SN			= 1,
						MVC_CMN_L_MODULE_TYPE		= 2,
						MVC_CMN_L_CONNECT_APP_ID	= 3,
						MVC_CMN_L_CONNECT_TYPE		= 4,
						MVC_CMN_L_CUSTOM_ID			= 5,
						MVC_CMN_L_APP_ID			= 6,
						MVC_CMN_L_EQUIPMENT_ID		= 7,
						MVC_CMN_L_NAME				= 8,
						MVC_CMN_L_RACK				= 9,
						MVC_CMN_L_CHASSIS			= 10,
						MVC_CMN_L_MODULE			= 11,
						MVC_CMN_L_PLACE				= 12,
						MVC_CMN_L_EL_RANGE			= 13,
						MVC_CMN_L_EN_RANGE			= 14,
						MVC_CMN_L_EL_NOMINAL		= 15,
						MVC_CMN_L_EN_NOMINAL		= 16,
						MVC_CMN_L_PERCENT			= 17,
						MVC_CMN_L_EL_MEASURE		= 18,
						MVC_CMN_L_EN_MEASURE		= 19,
						MVC_CMN_L_SYSTEM_DEVIATION	= 20,
						MVC_CMN_L_SD				= 21,
						MVC_CMN_L_LOW_BORDER		= 22,
						MVC_CMN_L_HIGH_BORDER		= 23,
						MVC_CMN_L_UNCERTAINTY		= 24,
						MVC_CMN_L_VALUE_COUNT		= 25,
						MVC_CMN_L_VALUE_0			= 26,
						MVC_CMN_L_VALUE_1			= 27,
						MVC_CMN_L_VALUE_2			= 28,
						MVC_CMN_L_VALUE_3			= 29,
						MVC_CMN_L_VALUE_4			= 30,
						MVC_CMN_L_VALUE_5			= 31,
						MVC_CMN_L_VALUE_6			= 32,
						MVC_CMN_L_VALUE_7			= 33,
						MVC_CMN_L_VALUE_8			= 34,
						MVC_CMN_L_VALUE_9			= 35,
						MVC_CMN_L_VALUE_10			= 36,
						MVC_CMN_L_VALUE_11			= 37,
						MVC_CMN_L_VALUE_12			= 38,
						MVC_CMN_L_VALUE_13			= 39,
						MVC_CMN_L_VALUE_14			= 40,
						MVC_CMN_L_VALUE_15			= 41,
						MVC_CMN_L_VALUE_16			= 42,
						MVC_CMN_L_VALUE_17			= 43,
						MVC_CMN_L_VALUE_18			= 44,
						MVC_CMN_L_VALUE_19			= 45,
						MVC_CMN_L_ERROR				= 46,
						MVC_CMN_L_ERROR_LIMIT		= 47,
						MVC_CMN_L_ERROR_RESULT		= 48,
						MVC_CMN_L_MEASUREMENT_TIME	= 49,
						MVC_CMN_L_CALIBRATOR		= 50;

// ==============================================================================================
// Measure::Type::Comparators
//
const int				MVC_CMN_C_INDEX				= 0,
						MVC_CMN_C_MODULE_SN			= 1,
						MVC_CMN_C_MODULE_TYPE		= 2,
						MVC_CMN_C_CONNECT_APP_ID	= 3,
						MVC_CMN_C_CONNECT_TYPE		= 4,
						MVC_CMN_C_CUSTOM_ID			= 5,
						MVC_CMN_C_APP_ID			= 6,
						MVC_CMN_C_EQUIPMENT_ID		= 7,
						MVC_CMN_C_NAME				= 8,
						MVC_CMN_C_RACK				= 9,
						MVC_CMN_C_CHASSIS			= 10,
						MVC_CMN_C_MODULE			= 11,
						MVC_CMN_C_PLACE				= 12,
						MVC_CMN_C_EL_RANGE			= 13,
						MVC_CMN_C_EN_RANGE			= 14,
						MVC_CMN_C_SP_TYPE			= 15,
						MVC_CMN_C_CMP_TYPE			= 16,
						MVC_CMN_C_EL_NOMINAL		= 17,
						MVC_CMN_C_EN_NOMINAL		= 18,
						MVC_CMN_C_EL_MEASURE		= 19,
						MVC_CMN_C_EN_MEASURE		= 20,
						MVC_CMN_C_CMP_ID			= 21,
						MVC_CMN_C_OUT_ID			= 22,
						MVC_CMN_C_ERROR				= 23,
						MVC_CMN_C_ERROR_LIMIT		= 24,
						MVC_CMN_C_ERROR_RESULT		= 25,
						MVC_CMN_C_MEASUREMENT_TIME	= 26,
						MVC_CMN_C_CALIBRATOR		= 27;

// ==============================================================================================

#endif // MEASUREVIEWHEADER_H
