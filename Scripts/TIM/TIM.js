// Generate configuration for module TIM
//
//
function generate_tim(confFirmware, module, LMNumber, frame, log, signalSet, opticModuleStorage)
{
	...
	let tsConstant = 150 * 0.001;	// 150 ms
    
    let defaultTf = valToADC(0/*5000*/ * 0.000001 / tsConstant, 0, 65535, 0, 0xffff);
    let defaultHighBound = 1195;
    let defaultLowBound = -205;
	let defaultK1 = 1.0;
	let defaultK2 = 0.0;
    ...
    for (let i = 0; i < TIMSignalMaxCount; i++)
    {
        ...
        else
        {
			...
			let highEngineeringUnits = signalA.highEngineeringUnits();
			let lowEngineeringUnits = signalA.lowEngineeringUnits();
			...
			let highEngineeringUnitsB = signalB.highEngineeringUnits();
			let lowEngineeringUnitsB = signalB.lowEngineeringUnits();
			...
			let highPhysical = unitsConvertor.electricToPhysical_ThermoCouple(electricHighLimit, electricLowLimit, electricHighLimit, electricUnit, sensorType);
			let lowPhysical = unitsConvertor.electricToPhysical_ThermoCouple(electricLowLimit, electricLowLimit, electricHighLimit, electricUnit, sensorType);
			...
			switch (sensorType)
			{
				...
				break;
				default:
						log.errINT1001("Unknown T sensor type " + sensorType + " in " + signalStrIdA);
			}
