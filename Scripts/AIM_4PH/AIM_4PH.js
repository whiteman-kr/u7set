...
//
//
function generate_aim4ph(confFirmware, module, LMNumber, frame, log, signalSet, opticModuleStorage)
{
	if (module.propertyValue("EquipmentID") == undefined)
	{
		log.errCFG3000("EquipmentID", "Module_AIM4PH");
		return false;
	}
    ...
    for (let i = 0; i < AIMSignalMaxCount; i++)
    {
        ...
        else
        {
			...
			let rload = signalA.propertyValue("Rload_Ohm");
			if (rload == undefined) 
			{
				log.errCFG3000("Rload_Ohm", signalStrIdA);
				return false;
			}
			...
			let rloadB = signalB.propertyValue("Rload_Ohm");
			if (rloadB == undefined) 
			{
				log.errCFG3000("Rload_Ohm", signalStrIdB);
				return false;
			}
			...
			if (rload != rloadB)
			{
				log.errCFG3028(signalStrIdA, signalStrIdB, module.propertyValue("EquipmentID"), "Rload_Ohm");
			}
			...
			let highPhysical = unitsConvertor.electricToPhysical_Input(electricHighLimit, electricLowLimit, electricHighLimit, electricUnit, sensorType, rload);
			let lowPhysical = unitsConvertor.electricToPhysical_Input(electricLowLimit, electricLowLimit, electricHighLimit, electricUnit, sensorType, rload);
			...
			if (setDataFloat(confFirmware, log, LMNumber, equipmentID, frame, ptr, "K2", k2) == false)         // K2
			{
				return false;
			}
            ptr += 4;
