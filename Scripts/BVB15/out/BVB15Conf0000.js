...
function generate_bvb15_rev1(builder, root, module, confFirmware, log, signalSet, subsystemStorage, opticModuleStorage, logicModuleDescription) {
    ...
    {
        var checkProperties_1 = ["AppDataServiceID", "AppDataEnable", "AppDataIP", "AppDataPort",
            "DiagDataServiceID", "DiagDataEnable", "DiagDataIP", "DiagDataPort",
            "OverrideAppDataWordCount", "OverrideDiagDataWordCount"];
        for (var cp = 0; cp < checkProperties_1.length; cp++) {
            if (ethernetController.propertyValue(checkProperties_1[cp]) == undefined) {
                log.errCFG3000(checkProperties_1[cp], ethernetController.equipmentId);
                return false;
            }
        }
    }
    ...
    for (var s = 0; s < 2; s++) {
        ...
        if (ethernetController.propertyBool(servicesName[s] + "DataEnable") == true) {
            ...
            if (serviceObject == null || serviceObject.isSoftware() == false) {
                ...
                //
                var checkProperties_2 = ["DataReceivingIP", "DataReceivingPort"];
                for (var cp = 0; cp < checkProperties_2.length; cp++) {
                    if (service.propertyValue(servicesName[s] + checkProperties_2[cp]) == undefined) {
                        log.errCFG3000(servicesName[s] + checkProperties_2[cp], service.equipmentId);
                        return false;
                    }
                }
...
function generate_niosConfiguration(confFirmware, log, frame, module, LMNumber, opticModuleStorage, logicModuleDescription) {
    ...
    for (var i = 0; i < chassis.childrenCount; i++) {
        ...
        blockPtr += 2;
        // TxAppDataSize
        var appWordsIoCount = ioModule.propertyInt("TxAppDataSize");
