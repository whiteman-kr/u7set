// Generate configuration for module MPS-24
//
//
		
function generate_mps24(confFirmware, module, LMNumber, frame, log, signalSet, opticModuleStorage)
{
	var Mode_420mA = 1;
	var Mode_020mA = 4;
	var Mode_024mA = 5;
	...
	var defaultFlags = [0x014, 0x2, 0x014, 0x10, 0xe, 0xe, 0xe, 0xe, 0xe, 0xe];
    ...
    var defaultHighBound = [24.576, 100, 24.576, 78.125];
    var defaultLowBound = [0, 0, 0, -78.125];
	var defaultK1 = 1.0;
	var defaultK2 = 0.0;
    ...
    for (var i = 0; i < moduleSignalsCount; i++)
    {
        ...
        else
        {
			...
			if (i == 0 || i == 2)
			{
				...
				//highBound = highBound_I(signal);
				//lowBound = lowBound_I(signal);
			}
			if (i == 1)
			{
				// R
			
				var connectionType = signal.propertyValue("ConnectionType");
				if (connectionType == undefined) 
				{
					log.errCFG3000("ConnectionType", signalStrId);
					return false;
				}
				...
				flags |= (connectionType << 1);
				
				var sensorTypeCode = 0;
					
				var NoSensor=0;
				var Ohm_Pt50_W1385=3;
				var Ohm_Pt100_W1385=4;
				var Ohm_Cu_50_W1428=5;
				var Ohm_Cu_50_W1426=7;
				var Ohm_Cu23=10;
					
				switch (sensorType)
				{
					case NoSensor:			sensorTypeCode = 0;	break;
					case Ohm_Pt50_W1385:	sensorTypeCode = 2;	break;
					case Ohm_Pt100_W1385:	sensorTypeCode = 1;	break;
					case Ohm_Cu_50_W1428:	sensorTypeCode = 3;	break;
					case Ohm_Cu23:			sensorTypeCode = 4;	break;
					case Ohm_Cu_50_W1426:	sensorTypeCode = 5;	break;
					default:
							log.errINT1001("Unknown R sensor type " + sensorType + " in " + signalStrId);
							return false;
				}
				
				flags |= (sensorTypeCode << 2);
