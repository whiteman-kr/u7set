...
//
"use strict";
let FamilyMSO4 = 0x1100;
let VersionMSO4 = 0x00c0;
...
let configScriptVersion = 6; // ScriptDeviceObject is used
...
function generate_mso3_rev1(builder, module, root, confFirmware, log, signalSet, subsystemStorage, opticModuleStorage, logicModuleDescription) {
    ...
    let configFrameCount = 5; // number of frames in each configuration
    ...
    let lanControllerCount = logicModuleDescription.Lan_ControllerCount;
    for (let l = 0; l < lanControllerCount; l++) {
        let lanType = logicModuleDescription.jsLanControllerType(l);
        let lanPlace = logicModuleDescription.jsLanControllerPlace(l);
    ...
    let txRxConfigFrame = lanConfigFrame;
...
function generateLanTuningController(ethernetcontrollerID, confFirmware, root, log, frame, module) {
    ...
    if (generate_LANConfiguration(confFirmware, log, frame, module, ethernetController, controllerTuningWordsCount, tuningIP, tuningPort, tuningServiceIP, tuningServicePort, tuningDataID, 0, 0, 0, 0, 0, 0) == false) //Subnet2 is not used
     {
        return false;
    }
    return true;
...
function generateLanAppDiagController(ethernetcontrollerID, confFirmware, root, log, frame, module, diagWordsCount) {
    ...
    let controllerAppWordsCount = module.propertyInt("AppLANDataSize");
...
function generate_niosConfiguration(confFirmware, log, frame, module, LMNumber, opticModuleStorage, logicModuleDescription) {
    ...
    ptr += 2;
    // Blocks[]
    let blocksPtr = ptr; // bptr is a pointer to Blocks [] array
    let blocksCount = 0;
    let txAddrSize = 128;
    let rxAddrSize = 128;
    for (let i = 0; i < chassis.childrenCount; i++) {
        ...
        let ioEquipmentID = ioModule.equipmentId;
        let checkProperties = ["ModuleVersion", "Place", "PresetName", "ConfigurationScript", "TxDiagDataSize", "TxAppDataSize", "Configuration"];
        for (let cp = 0; cp < checkProperties.length; cp++) {
        ...
        let blockPtr = blocksPtr + (blocksCount * structSize);
        blocksCount++;
        ...
        blockPtr += 2;
        // RxAddr
        let rxAddr = rxAddrSize * (ioPlace - 1);
        ...
        blockPtr += 2;
        // RxAppDataSize
        let rxAppDataSize = ioModule.propertyInt("RxAppDataSize") * 2;
        ...
        blockPtr += 2;
        // TxAddr
        let txAddr = txAddrSize * (ioPlace - 1);
        ...
        blockPtr += 2;
        // TxAppDataSize
        let txAppDataSize = ioModule.propertyInt("TxAppDataSize") * 2;
        ...
        blockPtr += 2;
        // Configuration
        let configurationCode = ioModule.propertyInt("Configuration");
