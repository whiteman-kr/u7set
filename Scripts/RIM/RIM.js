...
function generate_rim(confFirmware, module, LMNumber, frame, log, signalSet, opticModuleStorage)
{
	...
	{
		log.errCFG3000("EquipmentID", "Module_RIM");
		return false;
	}
    ...
    for (let i = 0; i < RIMSignalMaxCount; i++)
    {
        ...
        if (signalA == null)
        {
            ...
            if (setDataFloat(confFirmware, log, LMNumber, equipmentID, frame, ptr, "R0_Ohm", defaultR0) == false)          // R0
			{
				return false;
			}
			ptr += 4;	// Reserved
			
            if (setData16(confFirmware, log, LMNumber, equipmentID, frame, ptr, "WordOfFlags", defaultWordOfFlags) == false)      // InA DefaultWordOfFlags
			{
				return false;
			}
            ptr += 2;
        ...
        else
        {
			...
			if (R0A != R0B)
			{
				log.errCFG3028(signalStrIdA, signalStrIdB, module.propertyValue("EquipmentID"), "R0_Ohm");
			}

			if (PGAGainA != PGAGainB)
			{
				log.errCFG3028(signalStrIdA, signalStrIdB, module.propertyValue("EquipmentID"), "PGAGain");
			}

			if (RTDConfModeA != RTDConfModeB)
			{
				log.errCFG3028(signalStrIdA, signalStrIdB, module.propertyValue("EquipmentID"), "RTDConfMode");
			}
            ...
            if (setDataFloat(confFirmware, log, LMNumber, equipmentID, frame, ptr, "R0_Ohm", R0A) == false)          // InA Low Bound
			{
				return false;
			}
			ptr += 4;
