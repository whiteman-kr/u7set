...
void MonitorSchemaView::updateConfiguration(const MonitorConfigSettings& configuration)
{
	qDebug() << "MonitorSchemaView::updateConfiguration()";
...
void MonitorSchemaView::paintEvent(QPaintEvent* event)
{
	...
	// details.
	//
	if (int cid = monitorSchemaManager()->configController().configurationId(); cid != m_configurationId)
	{
		updateConfiguration(monitorSchemaManager()->configController().configuration());
	}
