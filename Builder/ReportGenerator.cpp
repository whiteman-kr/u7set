	...
	*/
	...
	void SchemasReportGenerator::exportAllSchemasToAlbums()
	{
		...
		try
		{
			...
			for (SchemaFilesGroup& sfg : schemaFilesGroups)
			{
				...
				{
					QMutexLocker l(&m_statisticsMutex);
					m_currentSchemaType = sfg.caption;
				}
	...
	void SchemasReportGenerator::renderSchemas(const SchemaFilesGroup& sfg)
	{
		...
		//
		{
			...
			m_currentSchemaType = sfg.caption;
			m_schemaIndex = 0;
			m_schemasCount = static_cast<int>(sfg.schemas.size());
		}
...

		{
			...
			for (auto it = sfg.schemas.begin(); it != sfg.schemas.end(); it++)
			{
				...
				QTextDocument emptyTextDocument;
				printDocument(&pdfWriter, &emptyTextDocument, &painter, tr("Schema: %1").arg(schemaId), nullptr, nullptr, 0);

				printSchema(&pdfWriter, &painter, schema, {}, {});
			}
		}
		...
		else
		{
			// Move buffer to output data map
			//
			m_outputData[sfg.caption + ".pdf"] = std::move(buffer.data());
		}
	}
