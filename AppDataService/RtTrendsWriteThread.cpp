...
//QString RtThrendsWriteThread::m_format1("row(%1,%2,%3,%4,%5)::AppSignalState");
//QString ArchWriteThreadWorker::m_format2(",row(%1,%2,%3,%4,%5)::AppSignalState");


RtTrendsWriteThread::RtTrendsWriteThread(const HostAddressPort& dbHost,
...
void RtTrendsWriteThread::run()
{
	...
	do
	{
		...
		{
			result = tryConnectToDb();

			if (result == true)
			{
				DEBUG_LOG_MSG(m_logger, "RtTrendsWriteThread: database is opened");
			}
			else
			{
				msleep(1000);
		...
		result = writeStatesToArchive();

		if (result == false)
		{
			// no states has been written
			msleep(10);
		}
	}
	while(isQuitRequested() == false);

	DEBUG_LOG_MSG(m_logger, "RtTrendsWriteThread is finished");
}


bool RtTrendsWriteThread::tryConnectToDb()
{
	if (m_db.isOpen() == true)
	{
		return true;
