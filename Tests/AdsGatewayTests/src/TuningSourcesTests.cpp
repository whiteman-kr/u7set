...
TEST(TuningSourcesTests, ParsesDefaultProfileAndSignals)
{
	...
	//
	{
		...
		ASSERT_TRUE(source4th.signals.contains(Radiy::calcHash("#TEST_DER_V5_T1_KD")));

		const auto& sp = source4th.signals.at(Radiy::calcHash("#TEST_DER_V5_T1_KD"));

		EXPECT_STREQ(sp.appSignalId, "#TEST_DER_V5_T1_KD");
		EXPECT_STREQ(sp.customSignalId, "TEST_DER_V5_T1_KD");
		EXPECT_STREQ(sp.caption, "App signal #TEST_DER_V5_T1_KD in schema TEST_DER_V5");
		EXPECT_STREQ(sp.equipmentId, "SYSTEMID_RACKID_FSCC01_MD00");
		EXPECT_STREQ(sp.lmEquipmentId, "SYSTEMID_RACKID_FSCC01_MD00");
