...
class SimDialogChooseTrendSignals : public QDialog
{
	Q_OBJECT

public:
	SimDialogChooseTrendSignals(Sim::AppSignalManager* signalManager,
								std::vector<TrendLib::TrendSignalParam>& trendSignals,
								QWidget* parent);
	virtual ~SimDialogChooseTrendSignals();
...
class SimFilteredTrendSignalsModel : public QAbstractTableModel
{
	Q_OBJECT

public:
	SimFilteredTrendSignalsModel(const std::vector<AppSignalParam>& signalss, QObject* parent);
