...
{
	for (const std::shared_ptr<Hardware::DiagSignalType>& dst : objectsVector())
	{
		Q_ASSERT(dst);
...
bool DiagSignalTypesStorage::load(QString* errorMessage)
{
	...
	bool ok = m_db->getFileList(&fileList, fileTypeId(), Db::File::DiagSignalTypeFileExtension, true, nullptr);
	if (ok == false)
	{
...
bool DiagSignalTypesStorage::save(const QUuid& uuid, QString* errorMessage)
{
	...
	if (fi.isNull() == true)
	{
		...
		if (m_db->addFile(file, fileTypeId(), nullptr) == false)
		{
			*errorMessage = m_db->lastError();
			return false;
		}
