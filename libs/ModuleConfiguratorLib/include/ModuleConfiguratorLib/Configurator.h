...
#pragma pack(pop)
	...
	protected:
		...
		//	bool send(HANDLE hDevice, int moduleUartId, ConfigureCommand opcode, uint16_t frameIndex, uint16_t blockSize, const
		//std::vector<uint8_t>& requestData, CONF_HEADER* pReceivedHeader, std::vector<uint8_t>* replyData);

		bool loadBinaryFileWorker(const QString& fileName, ModuleFirmwareStorage* storage, bool loadBinaryData);
		void uploadFirmwareWorker(ModuleFirmwareStorage* storage,
								  const QString& subsystemId,
								  std::optional<std::vector<int>> selectedUarts);

		void readServiceInformationWorker(int param);
		bool readFirmwareWorker(std::vector<ModuleFirmwareData>* firmwareDataArray,
								bool logIdentificationData,
								int maxFrameCount,
								std::optional<std::vector<int>> selectedUarts);

		void dumpIdentificationData(const std::vector<quint8>& identificationData, int blockSize, QStringList& out, QUuid* moduleUartUuid, bool* error);
