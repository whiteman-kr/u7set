...
namespace ModuleConfiguratorLib
{
	...
	void CONF_IDENTIFICATION_DATA_V1::dump(QStringList& out) const
	{
		...
		out << "__Date: " +
				   DateTimeToString::dateTimeSec(QDateTime().fromSecsSinceEpoch(static_cast<uint>(firstConfiguration.date)));
		...
		out << "__Date: " + 
			DateTimeToString::dateTimeSec(QDateTime().fromSecsSinceEpoch(static_cast<uint>(lastConfiguration.date)));
	...
	void CONF_IDENTIFICATION_DATA_V1::createNextConfiguration()
	{
		...
		CONF_IDENTIFICATION_DATA_V1::CONF_IDENTIFICATION_RECORD lastConfiguration =
			CONF_IDENTIFICATION_DATA_V1::CONF_IDENTIFICATION_RECORD();
	...
	void CONF_IDENTIFICATION_DATA_V2::dump(QStringList& out) const
	{
		...
		out << "First time configured: ";
		out << "__Date: " + DateTimeToString::dateTimeSec(QDateTime().fromSecsSinceEpoch(static_cast<uint>(firstConfiguration.date)));
		out << "__Host: " + QString(firstConfiguration.host);
		...
		out << "Last time configured: ";
		out << "__Date: " + DateTimeToString::dateTimeSec(QDateTime().fromSecsSinceEpoch(static_cast<uint>(lastConfiguration.date)));
		out << "__Host: " + QString(lastConfiguration.host);
	...
	bool Configurator::openConnection()
	{
		...
		//
		{
			...
			{
				m_Log->writeError("Current license does not allow the use of the module configurator.");
				return false;
			}
		}
							...
							std::vector<quint8>* replyData)
	{
		...
		default:
			assert(false);
			m_Log->writeError(__FUNCTION__ + tr(" Unknown command %1.").arg(static_cast<int>(opcode)));
			return false;
		}
		...
		if (opcode == ConfigureCommand::Nop2 && recBuffer.size() == headerSize)
		{
			expecetedDataBytes =
				0; // We have received correct header and 0 data bytes on Nop2 request, possibly this command is not supported
		}
		else
		{
			if (recBuffer.size() != recSize)
		...
		*pReceivedHeader = *reinterpret_cast<decltype(pReceivedHeader)>(recHeaderBuffer.data());
		...
		{
			m_Log->writeError(tr("Wrong CRC, received value is: ") + QString().setNum(pReceivedHeader->crc64, 16).rightJustified(16, '0') +
							  ".");
											...
											std::optional<std::vector<int>> selectedUarts)
	{
		...
		try
		{
			...
			if (moduleUarts.size() == 1 && (moduleUarts[0] & ConfigurationUartMask) == ConfigurationUartValue)
			{
			...
			for (int moduleUartId : moduleUarts)
			{
				...
				{
					throw tr("EEPROM Frame size is to small, requeried at least %1, but current frame size is %2.")
						.arg(confFrameDataSize)
						.arg(payloadSize);
				}
				...
				case 1:
					{
						...
						if (bool sendOk = send(moduleUartId,
											   ConfigureCommand::Read,
											   IdentificationFrameIndex,
											   blockSize,
											   std::vector<uint8_t>(),
											   &readReceivedHeader,
											   &identificationData);
				...
				CONF_IDENTIFICATION_DATA* pReadIdentificationStruct =
					reinterpret_cast<CONF_IDENTIFICATION_DATA*>(identificationData.data());

				// Check license
				//
				{
					LicenseLib::AppLicenser licenser;
					auto lc = licenser.validator().validateModuleConfiguratorModule(pReadIdentificationStruct->moduleUuid.toQUuid());
					if (lc != LicenseLib::ValidationResult::Valid)
					{
						throw tr("Current license does not allow configuring this module, %1.").arg(pReadIdentificationStruct->moduleUuid.toQUuid().toString());
					}
				}
					...
					pReadIdentificationStruct->version != CONF_IDENTIFICATION_DATA::structVersion())
				{
					if (pReadIdentificationStruct->marker == IdentificationStructMarker)
					{
						m_Log->writeMessage(tr("Upgrading CONF_IDENTIFICATION_DATA struct version: %1 -> %2")
												.arg(pReadIdentificationStruct->version)
												.arg(CONF_IDENTIFICATION_DATA::structVersion()));
					}

					if (sizeof(CONF_IDENTIFICATION_DATA) > blockSize)
					{
						throw tr("CONF_IDENTIFICATION_DATA struct size (%1) is bigger than blockSize (%2).")
							.arg(sizeof(CONF_IDENTIFICATION_DATA))
							.arg(blockSize);
					}
				...
				if (send(moduleUartId,
						 ConfigureCommand::Write,
						 IdentificationFrameIndex,
						 blockSize,
						 identificationData,
						 &replyIdentificationHeader,
						 &replyData) == false)
				{
					throw tr("Communication error.");
				}
				...
				{
					throw tr("Command Write reply error. Different header version, expected 1, received ") +
						QString().setNum(replyIdentificationHeader.version) + ".";
				}
				...
				if (verify() == true)
				{
					...
					{
						throw(tr("Send identificationData size does not match received data size in frame %1.")
								  .arg(IdentificationFrameIndex));
					}
					for (int v = 0; v < identificationData.size(); v++)
					{
						if (readData[v] != identificationData[v])
						{
							throw(tr("Sent identificationData does not match received data size in frame %1, offset %2.")
									  .arg(IdentificationFrameIndex)
									  .arg(v));
						}
					}
				...
				case 1:
					{
						for (uint16_t i = 0; i < conf.eepromFrameCount(moduleUartId); i++)
						{
							...
							{
								throw tr("Frame %1 size (%2) does not match module's BlockSize (%3).")
									.arg(i)
									.arg(frameData.size())
									.arg(blockSize);
							}
							...
							bool sendResult = send(moduleUartId, ConfigureCommand::Write, frameIndex, blockSize, frameData, &replyHeader, &replyBuffer);
							...
							{
								throw tr("Command Write reply error. Different header version, expected 1, received ") +
									QString().setNum(replyHeader.version) + ".";
							}
							...
							if (verify() == true)
							{
								...
								for (int v = 0; v < frameData.size(); v++)
								{
									...
									{
										throw(tr("Send data does not match received data size in frame %1, offset %2.")
												  .arg(frameIndex)
												  .arg(v));
									}
								}
	...
	void Configurator::readServiceInformationWorker(int /*param*/)
	{
		...
		try
		{
			...
			case 1:
				{
					...
					//
					QStringList dumpLog;
					QUuid muduleUartUuid;

					dumpIdentificationData(identificationData, blockSize, dumpLog, &muduleUartUuid, &identificationError);

					// Check license
					//
					LicenseLib::AppLicenser licenser;
					auto lc = licenser.validator().validateModuleConfiguratorModule(muduleUartUuid);
					if (lc != LicenseLib::ValidationResult::Valid)
					{
						throw tr("Current license does not allow configuring this module, %1.").arg(muduleUartUuid.toString());
					}
			...
			if (send(moduleUartId, ConfigureCommand::Read, ConfiguartionFrameIndex, blockSize, std::vector<quint8>(), &readReceivedHeader, &readData) ==
				false)
			{
				throw tr("Communication error.");
			}
	...
	bool Configurator::readFirmwareWorker(std::vector<ModuleFirmwareData>* firmwareDataArray,
										  bool logIdentificationData,
										  int maxFrameCount,
										  std::optional<std::vector<int>> selectedUarts)
	{
				...
				case 1:
					{
						for (decltype(CONF_HEADER_V1().frameIndex) i = 0; i < blockCount; i++)
						{
							...
							if (bool sendOk = send(moduleUartId, ConfigureCommand::Read, i, blockSize, std::vector<quint8>(), &readReceivedHeader, &readData);
								sendOk == false)
							{
								throw tr("Communication error.");
							}
							...
							if (i == 0 && logIdentificationData == true)
							{
								...
								dumpIdentificationData(readData, blockSize, dumpLog, &muduleUartUuid , &identificationError);
								...
								if (lc != LicenseLib::ValidationResult::Valid)
								{
									QString e = tr("Current license does not allow configuring this module, %1.").arg(muduleUartUuid.toString());
									m_Log->writeError(e);
									break;
								}
							}
							...
							case 1:
								...
								break;
							default:
								m_Log->writeError(tr("Unsupported protocol version, module protocol version: ") +
												  QString().setNum(protocolVersion) + tr(", the maximum supported version: ") +
												  QString().setNum(ProtocolMaxVersion) + ".");
								throw tr("Communication error.");
							}
						}
					}
	...
	void Configurator::dumpIdentificationData(const std::vector<quint8>& identificationData, int blockSize, QStringList& out, QUuid* moduleUartUuid, bool* error)
	{
		if (error == nullptr || moduleUartUuid == nullptr)
		{
			Q_ASSERT(error);
			Q_ASSERT(moduleUartUuid);
			return;
		}
		...
		*error = false;
		...
		const CONF_IDENTIFICATION_DATA* pReadIdentificationStruct =
			reinterpret_cast<const CONF_IDENTIFICATION_DATA*>(identificationData.data());
		...
		if (pReadIdentificationStruct->marker == IdentificationStructMarker)
		{
			...
			case 1:
				{
					const CONF_IDENTIFICATION_DATA_V1* pReadIdentificationStruct_v1 =
						reinterpret_cast<const CONF_IDENTIFICATION_DATA_V1*>(identificationData.data());
					
					*moduleUartUuid = pReadIdentificationStruct_v1->moduleUuid.toQUuid();
			...
			case 2:
				{
					const CONF_IDENTIFICATION_DATA_V2* pReadIdentificationStruct_v2 =
						reinterpret_cast<const CONF_IDENTIFICATION_DATA_V2*>(identificationData.data());

					*moduleUartUuid = pReadIdentificationStruct_v2->moduleUuid.toQUuid();
			...
			case 1:
				{
					...
					if (send(moduleUartId, ConfigureCommand::Write, ConfiguartionFrameIndex, blockSize, writeData, &replyHeader, &replyDataV1) == false)
					{
						throw tr("Communication error.");
					}
		...
		for (const ModuleFirmwareData& fd : fdArray)
		{
			...
			for (int f = 0; f < fd.frames.size(); f++)
			{
				...
				if (f == 0)
				{
					...
					dumpIdentificationData(frame, static_cast<int>(frame.size()), dumpLog, &muduleUartUuid, &identificationError);
			...
			case 1:
				{
					...
					if (romSize % blockSize != 0)
					{
						throw tr("Flash memory block count is not an integral multiple.");
					}
				...
				case 1:
					{
						for (decltype(CONF_HEADER_V1().frameIndex) i = 0; i < blockCount; i++)
						{
							...
							if (bool sendOk = send(moduleUartId, ConfigureCommand::Write, i, blockSize, writeData, &replyHeader, &replyData); sendOk == false)
							{
								throw tr("Communication error.");
							}
