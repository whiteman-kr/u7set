...
namespace UiLib
{
	...
	private:
		IExportPrint& m_exportPrintInterface;
	...
	ExportPrintPrivate::ExportPrintPrivate(IExportPrint& exportPrintInterface, QWidget* parent) :
		m_exportPrintInterface{exportPrintInterface},
		m_parent{parent}
	{
	}

	void ExportPrintPrivate::printTable(QTableView* tableView)
	{
	...
	void ExportPrintPrivate::exportTable(QTableView* tableView, QString fileName, QString extension)
	{
	...
	void ExportPrintPrivate::generateHeader(QTextCursor& cursor)
	{
		m_exportPrintInterface.generateHeader(cursor);
	}

	bool ExportPrintPrivate::exportToTextDocument(QTableView* tableView, QTextDocument* doc, bool onlySelectedRows)
	{
		...
		// re-init rowCount if printing only selected rows
		//
		...
		QProgressDialog progressDialog(QObject::tr("Generating report..."), QObject::tr("Cancel"), 0, rowCount, m_parent);
		...
		{
			cursor.insertText(QObject::tr("Warning: Only first %1 of %2 records present in generated report.\n")
								  .arg(rowCount)
								  .arg(tableView->model()->rowCount()));
		}
	...
	bool ExportPrintPrivate::saveArchiveWithDocWriter(QTableView* tableView, QString fileName, QString format)
	{
		if (tableView == nullptr || fileName.isEmpty() == true)
		{
			Q_ASSERT(tableView);
		...
		if (model->rowCount() > m_maxReportStates)
		{
			QMessageBox::warning(
				m_parent,
				qAppName(),
				QObject::tr("Too many archive records, only first %1 states will appear in the generated report.").arg(m_maxReportStates));
		}
		...
		{
			QMessageBox::critical(m_parent, qAppName(), QObject::tr("Unsupported file format."));
			return false;
		}
	...
	bool ExportPrintPrivate::saveArchiveToCsv(QTableView* tableView, QString fileName)
	{
		if (tableView == nullptr || fileName.isEmpty() == true)
		{
			Q_ASSERT(tableView);
		...
		if (model->rowCount() > m_maxReportStatesForCsv)
		{
			QMessageBox::warning(m_parent,
								 qAppName(),
								 QObject::tr("Too many archive records, only first %1 states will appear in the generated report.")
									 .arg(m_maxReportStatesForCsv));
		}
		...
		if (ok == false)
		{
			QMessageBox::critical(m_parent, qAppName(), QObject::tr("Cannot open file %1 for writing.").arg(fileName));
			return false;
		}
		...
		for (int row = 0; row < rowCount; row++)
		{
			...
			if (row % 10000 == 0)
			{
				progressDialog.setLabelText(QObject::tr("Generating report... %1/%2").arg(row).arg(rowCount));
			}
...
namespace UiLib
{
	ExportPrint::ExportPrint(QWidget* parent) :
		m_impl{std::make_unique<ExportPrintPrivate>(*this, parent)}
	{
	}

	ExportPrint::~ExportPrint() = default;

	void ExportPrint::printTable(QTableView* tableView)
	{
		return m_impl->printTable(tableView);
	}

	void ExportPrint::exportTable(QTableView* tableView, QString fileName, QString extension)
	{
		return m_impl->exportTable(tableView, fileName, extension);
	}
