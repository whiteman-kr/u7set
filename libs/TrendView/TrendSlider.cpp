...
namespace TrendLib
{
	...
	void TrendSliderRailSubcontrol::mousePressEvent(QMouseEvent* event)
	{
		if ((event->button() & Qt::LeftButton) != 0)
		{
			...
			if (sliderSubcontrolRect.contains(event->pos()) == true)
			{
				...
				//
				grabMouse();
				m_railLastMousePos = event->pos().x();
			...
			else
			{
				...
				qint64 newValue = qRound64(m_min + static_cast<double>(event->pos().x()) / slideAreaWidth * (m_max - m_min));
	...
	{
		if ((event->button() & Qt::LeftButton) != 0 && QWidget::mouseGrabber() == this)
		{
			releaseMouse();
		}
	...
	{
		if (QWidget::mouseGrabber() == this && (m_railLastMousePos - event->pos().x()) != 0)
		{
			// Move cursor to mouse position
			//
			if (rect().width() == 0) // avoid div by 0
			...
			double slideAreaWidth = rect().width() - sliderSubcontrolRect.width();
			qint64 newValue = qRound64(m_railPressMouseValue +
									   static_cast<double>(event->pos().x() - m_railLastMousePos) / slideAreaWidth * (m_max - m_min));
	...
	QRect TrendSliderRailSubcontrol::sliderRect() const
	{
		...
		else
		{
			...
			{
				sliderPos = static_cast<int>(static_cast<double>(m_value - m_min) / static_cast<double>(m_max - m_min) *
