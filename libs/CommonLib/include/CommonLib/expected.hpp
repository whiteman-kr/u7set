...
namespace tl
{
	...
	// in some cases. This is a check to workaround the common failing case.
		...
		struct is_nothrow_swappable
			: std::integral_constant<bool,
									 is_swappable<T, U>::value && ((decltype(detail::swap_adl_tests::uses_std<T, U>(0))::value &&
...
// no constexpr for GCC 4/5 bug
		...
		struct expected_operations_base : expected_storage_base<T, E>
		{
			...
			template<class U = T,
					 detail::enable_if_t<!std::is_nothrow_copy_constructible<U>::value && std::is_nothrow_move_constructible<U>::value>* =
						 nullptr>
			void assign(const expected_operations_base& rhs) noexcept
			{
				if (!this->m_has_val && rhs.m_has_val)
...
// move constructible
		...
		template<class T,
				 ...
				 bool = is_void_or<T,
						...
						TL_EXPECTED_IS_TRIVIALLY_COPY_ASSIGNABLE(E)::value && TL_EXPECTED_IS_TRIVIALLY_COPY_CONSTRUCTIBLE(E)::value &&
...
// move assignable
		...
		struct expected_move_assign_base<T, E, false> : expected_copy_assign_base<T, E>
		{
			...
			expected_move_assign_base& operator=(expected_move_assign_base&& rhs) noexcept(std::is_nothrow_move_constructible<T>::value &&
	...
	private:
		...
		void swap_where_only_one_has_value_and_t_is_not_void(expected& rhs, move_constructing_t_can_throw, e_is_nothrow_move_constructible)
		{
