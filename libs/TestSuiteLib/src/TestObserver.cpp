...
namespace TestSuite
{
								 ...
								 TrendLib::TrendStateItem /*maxState*/)
	{
		...
		if (m_waitPrivateData.initialConditionPtr != nullptr)
		{
			...
			auto it = std::find_if(signalData.begin(),
								   signalData.end(),
								   [hash = m_waitPrivateData.initialConditionPtr->appSignalHash](const TrendLib::RealtimeDataChunk& chunk)
								   {
			...
			if (it != signalData.end())
			{
				...
				for (const TrendLib::TrendStateItem& state : chunk.states)
				{
					...
					if (m_waitPrivateData.initialConditionPtr->met() == true)
					{
						...
						m_waitPrivateData.initialConditionPtr =
							nullptr; // This indicates that initial condition is was met, so we can leave the loop.
						...
						for (auto& expectation : m_expectations)
						{
							...
							for (auto statesIt = states.begin(); statesIt != states.end();)
							{
								...
								if (auto itNext = std::next(statesIt); itNext != states.end() && itNext->getTime(m_timeType) <= timePoint)
								{
		...
		//
		for (const std::list<TrendLib::RealtimeDataChunk>& signalData = data->signalData; const auto& chunk : signalData)
		{
			for (auto& e : m_expectations)
			{
	...
	bool TestObserver::waitPrivate(int initialExpectationId, int timeoutMs)
	{
		...
		while (deadline.hasExpired() == false)
		{
			...
			//
			{
				bool met = std::all_of(m_expectations.begin(),
									   m_expectations.end(),
									   [](const auto& e)
									   {
