...
namespace TestSuite
{
	TestLogItem::TestLogItem(int no, TestLogItemType type, const QString& text, const QString& tag) :
		m_no(no),
		m_time(QDateTime::currentDateTime().time()),
		m_type(type),
		m_message(text),
		m_tag(tag)
	{
	...
	QString TestLogItem::toText() const
	{
		...
		return QObject::tr("%1 | %2 | %3 | %4").arg(m_no, 4, 10, QChar('0')).arg(m_time.toString("hh:mm:ss:zzz")).arg(type).arg(m_message);
	}
	...
	QString TestLogItem::toHtml() const
	{
		...
		{
		case TestLogItemType::Text:
			result = QString("<font face=\"%1\" size=\"4\" color=black>%2</font>").arg(m_htmlFont).arg(m_message.toHtmlEscaped());
			break;
		case TestLogItemType::Message:
	...
	QStringList TestLogItem::toStringList() const
	{
		...
		result << m_time.toString("hh:mm:ss:zzz");
	...
	const QTime& TestLogItem::time() const
	{
		return m_time;
	}
	...
	bool TestLog::saveToCSV(const QString& fileName, QString* errorMsg) const
	{
		...
		for (const TestLogItem& item : items())
		{
	...
	bool TestLog::loadFromCSV(const QString& fileName, QString* errorMsg)
	{
		...
		while (in.atEnd() == false)
		{
			...
			bool lineReadOk = false;
			TestLogItem item = TestLogItem::fromStringList(in.readLine(), &lineReadOk);
