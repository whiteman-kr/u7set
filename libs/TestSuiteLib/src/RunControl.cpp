...
namespace TestSuite
{
	...
	void RunControlThread::taskPrepare()
	{
		if (m_inputController == nullptr)
		{
			...
			m_appLog.writeError(tr("Input controller is not initialized."));
			throw 1;
		}
		...
		std::optional<TestScript> globalScript = m_scriptProvider->getGloablScript();

		m_executors.clear();
		m_executors.reserve(scripts.size());
		...
		for (const auto& script : scripts)
		{
			...
			//
			const auto& scriptInfo = scriptRunner->scriptInfo();
			...
			{
				m_executors.push_back({std::move(testController), std::move(scriptRunner)});
			}
		}
	}
	...
	void RunControlThread::taskQueryPermission()
	{
		...
		for (const auto& e : m_executors)
		{
			auto& runner = e.scriptRunner;
			...
			auto checkTestExecutionTime = [&callFinishedCondVariable, &callFinished, scriptRunThread, &runner, fileName, this]() -> void
			{
				do
				{
					if (runner->testController().executionTimeout() > 0 &&
						status().duration().count() > runner->testController().executionTimeout())
					{
	...
	{
		connect(static_cast<RunControlThread*>(m_controlThread.get()),
				&RunControlThread::scriptPermissionChanged,
				[this](QString scriptFileName, bool result)
				{
					emit scriptPermissionChanged(scriptFileName, result);
				});

		connect(static_cast<RunControlThread*>(m_controlThread.get()),
				&RunControlThread::globalPermissionChanged,
				[this](bool result)
				{
		...
		connect(static_cast<RunControlThread*>(m_controlThread.get()),
				&RunControlThread::noPermissionsExist,
				[this]()
				{
					emit noPermissionsExist();
				});
