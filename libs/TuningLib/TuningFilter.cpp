...
namespace TuningFilters
{
	...
	namespace TuningTags
	{
		...
		static const QString tag_StatusBar = "StatusBar";
		static const QString tag_FilterTree = "FilterTree";
		...
		static const QLatin1String prop_CounterType = QLatin1String("CounterType");
	...
	TuningFilter::TuningFilter()
	{
		...
		auto propMask = ADD_PROPERTY_GETTER_SETTER(QString,
												   TuningTags::prop_CustomAppSignalMasks,
												   true,
												   TuningFilter::customAppSignalIDMask,
												   TuningFilter::setCustomAppSignalIDMask);
		propMask->setCategory("Masks");

		propMask = ADD_PROPERTY_GETTER_SETTER(QString,
											  TuningTags::prop_AppSignalMasks,
											  true,
											  TuningFilter::appSignalIDMask,
											  TuningFilter::setAppSignalIDMask);
		propMask->setCategory("Masks");

		propMask = ADD_PROPERTY_GETTER_SETTER(QString,
											  TuningTags::prop_EquipmentIDMasks,
											  true,
											  TuningFilter::equipmentIDMask,
											  TuningFilter::setEquipmentIDMask);
		propMask->setCategory("Masks");

		propMask = ADD_PROPERTY_GETTER_SETTER(QString,
											  TuningTags::prop_AppSignalTags,
											  true,
											  TuningFilter::appSignalTags,
											  TuningFilter::setAppSignalTags);
		propMask->setCategory("Masks");

		auto propUseColors =
			ADD_PROPERTY_GETTER_SETTER(bool, TuningTags::prop_UseColors, true, TuningFilter::useColors, TuningFilter::setUseColors);
		propUseColors->setCategory("Appearance");

		auto propBackColor =
			ADD_PROPERTY_GETTER_SETTER(QColor, TuningTags::prop_BackColor, true, TuningFilter::backColor, TuningFilter::setBackColor);
		propBackColor->setCategory("Appearance");

		auto propTextColor =
			ADD_PROPERTY_GETTER_SETTER(QColor, TuningTags::prop_TextColor, true, TuningFilter::textColor, TuningFilter::setTextColor);
		propTextColor->setCategory("Appearance");

		auto propBackSelectedColor = ADD_PROPERTY_GETTER_SETTER(QColor,
																TuningTags::prop_BackSelectedColor,
																true,
																TuningFilter::backSelectedColor,
																TuningFilter::setBackSelectedColor);
		propBackSelectedColor->setCategory("Appearance");

		auto propTextSelectedColor = ADD_PROPERTY_GETTER_SETTER(QColor,
																TuningTags::prop_TextSelectedColor,
																true,
																TuningFilter::textSelectedColor,
																TuningFilter::setTextSelectedColor);
		propTextSelectedColor->setCategory("Appearance");

		auto propBackAlertedColor = ADD_PROPERTY_GETTER_SETTER(QColor,
															   TuningTags::prop_BackAlertedColor,
															   true,
															   TuningFilter::backAlertedColor,
															   TuningFilter::setBackAlertedColor);
		propBackAlertedColor->setCategory("Appearance");

		auto propTextAlertedColor = ADD_PROPERTY_GETTER_SETTER(QColor,
															   TuningTags::prop_TextAlertedColor,
															   true,
															   TuningFilter::textAlertedColor,
															   TuningFilter::setTextAlertedColor);
		propTextAlertedColor->setCategory("Appearance");

		auto propHasCounter = ADD_PROPERTY_GETTER_SETTER(bool,
														 TuningTags::prop_HasDiscreteCounter,
														 true,
														 TuningFilter::hasDiscreteCounter,
														 TuningFilter::setHasDiscreteCounter);
		propHasCounter->setCategory("Functions");

		auto propTabValuesCount = ADD_PROPERTY_GETTER_SETTER(int,
															 TuningTags::prop_ValueColumnsCount,
															 true,
															 TuningFilter::valuesColumnCount,
															 TuningFilter::setValuesColumnCount);
		propTabValuesCount->setCategory(TuningTags::category_ValueColumns);

		auto propTabType =
			ADD_PROPERTY_GETTER_SETTER(TabType, TuningTags::prop_TabType, true, TuningFilter::tabType, TuningFilter::setTabType);
		propTabType->setCategory("Appearance");

		auto propCounterType = ADD_PROPERTY_GETTER_SETTER(CounterType,
														  TuningTags::prop_CounterType,
														  true,
														  TuningFilter::counterType,
														  TuningFilter::setCounterType);
		propCounterType->setCategory("Appearance");

		auto propSchemaId = ADD_PROPERTY_GETTER_SETTER(QString,
													   TuningTags::prop_StartSchemaId,
													   true,
													   TuningFilter::startSchemaId,
													   TuningFilter::setStartSchemaId);
		...
		auto propColumn = ADD_PROPERTY_GETTER_SETTER(bool,
													 TuningTags::prop_ColumnCustomAppId,
													 true,
													 TuningFilter::columnCustomAppId,
													 TuningFilter::setColumnCustomAppId);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn =
			ADD_PROPERTY_GETTER_SETTER(bool, TuningTags::prop_ColumnAppId, true, TuningFilter::columnAppId, TuningFilter::setColumnAppId);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn = ADD_PROPERTY_GETTER_SETTER(bool,
												TuningTags::prop_ColumnEquipmentId,
												true,
												TuningFilter::columnEquipmentId,
												TuningFilter::setColumnEquipmentId);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn = ADD_PROPERTY_GETTER_SETTER(bool,
												TuningTags::prop_ColumnCaption,
												true,
												TuningFilter::columnCaption,
												TuningFilter::setColumnCaption);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn =
			ADD_PROPERTY_GETTER_SETTER(bool, TuningTags::prop_ColumnUnits, true, TuningFilter::columnUnits, TuningFilter::setColumnUnits);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn =
			ADD_PROPERTY_GETTER_SETTER(bool, TuningTags::prop_ColumnType, true, TuningFilter::columnType, TuningFilter::setColumnType);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn = ADD_PROPERTY_GETTER_SETTER(bool,
												TuningTags::prop_ColumnLimits,
												true,
												TuningFilter::columnLimits,
												TuningFilter::setColumnLimits);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn = ADD_PROPERTY_GETTER_SETTER(bool,
												TuningTags::prop_ColumnDefault,
												true,
												TuningFilter::columnDefault,
												TuningFilter::setColumnDefault);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn =
			ADD_PROPERTY_GETTER_SETTER(bool, TuningTags::prop_ColumnValid, true, TuningFilter::columnValid, TuningFilter::setColumnValid);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);

		propColumn = ADD_PROPERTY_GETTER_SETTER(bool,
												TuningTags::prop_ColumnOutOfRange,
												true,
												TuningFilter::columnOutOfRange,
												TuningFilter::setColumnOutOfRange);
		propColumn->setCategory(TuningTags::category_Columns);
		propColumn->setViewOrder(order++);
	...
	*/
	...
	bool TuningFilter::load(QXmlStreamReader& reader)
	{
		if (isRoot() == false)
		{
			...
			if (reader.attributes().hasAttribute(TuningTags::prop_CounterType))
			{
				QString v = reader.attributes().value(TuningTags::prop_CounterType).toString();
				if (v == TuningTags::tag_StatusBar)
				{
					setCounterType(CounterType::StatusBar);
				}
				else
				{
					if (v == TuningTags::tag_FilterTree)
					{
		...
		do
		{
			...
			if (t == QXmlStreamReader::StartElement)
			{
				...
				if (source() != Source::User && tagName == TuningTags::tag_SignalHashes)
				{
				...
				if (source() != Source::User && tagName == TuningTags::tag_SourceHashes)
				{
					QByteArray data = QByteArray::fromBase64(reader.readElementText().toUtf8());
				...
				if (tagName == TuningTags::tag_Tree || tagName == TuningTags::tag_Tab || tagName == TuningTags::tag_Button ||
					tagName == TuningTags::tag_Counter || tagName == TuningTags::tag_SchemasTab)
				{
	...
	bool TuningFilter::save(QXmlStreamWriter& writer, Source saveSourceType) const
	{
		...
		{
			if (saveSourceType != Source::All && saveSourceType != source())
			{
				return true;
			}
			...
			else
			{
				...
				else
				{
					...
					else
					{
						if (isCounter())
						{
							writer.writeStartElement(TuningTags::tag_Counter);
						}
						else
						{
							if (isSchemasTab())
		...
		writer.writeAttribute(TuningTags::prop_CounterType, E::valueToString<CounterType>(static_cast<int>(counterType())));
	...
	void TuningFilter::checkSignals(const std::vector<Hash>& signalHashes,
	...
	{
		if (m_signalType == SignalType::All && filterSignalsCount() == 0 && m_cachedCustomAppSignalIDMasks.empty() == true &&
			m_cachedEquipmentIDMasks.empty() == true && m_cachedAppSignalIDMasks.empty() == true && m_cachedAppSignalTags.empty() == true)
		{
			return true;
		}
	...
	void TuningFilter::setCounterType(CounterType type)
	{
		m_counterType = type;
	}
	...
	bool TuningFilter::removeChild(int index)
	{
		if (index < 0 || index >= static_cast<int>(m_childFilters.size()))
		{
			Q_ASSERT(false);
			return false;
		}
	...
	{
		// Colors
		setPropertyVisible(TuningTags::prop_UseColors,
		...
		setPropertyVisible(TuningTags::prop_BackAlertedColor,
						   interfaceType() == InterfaceType::Button || interfaceType() == InterfaceType::Counter);
		setPropertyVisible(TuningTags::prop_TextAlertedColor,
		...
		setPropertyVisible(TuningTags::prop_HasDiscreteCounter,
	...
	void TuningFilterStorage::checkFilterSignals(const std::vector<Hash>& signalHashes,
												 std::vector<std::pair<QString, QString>>& notFoundSignalsAndFilters)
	{
		m_root->checkSignals(signalHashes, notFoundSignalsAndFilters);
	}

	void TuningFilterStorage::createSignalsAndEqipmentHashes(const ISignalManager& objects,
															 ...
															 TuningFilter::Source source)
	{
		...
		if (filter->parentFilter() == m_root.get() && filter->isCounter() == true &&
		...
		while (true)
		{
			if (source != TuningFilter::Source::All && source != filter->source())
			{
