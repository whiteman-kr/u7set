...
namespace Sim
{
		...
		m_log{m_scriptSimulator->log()}
	{
		...
		connect(this, &QThread::started, scriptSimulator->simulator(), &SimulatorPrivate::scriptStarted);
		connect(this, &QThread::finished, scriptSimulator->simulator(), &SimulatorPrivate::scriptFinished);
	...
	ScriptSimulator::ScriptSimulator(SimulatorPrivate* simulator, QObject* parent) :
		QObject(parent),
	...
	{
		if (m_simulator->overrideSignals().containsSignal(appSignalId) == false)
		{
	...
	ScriptLmDescription ScriptSimulator::scriptLmDescription(QString equipmentId)
	{
		...
		std::shared_ptr<LogicModuleImpl> lm = m_simulator->logicModule(equipmentId);
	...
	SimulatorPrivate* ScriptSimulator::simulator()
	{
		return m_simulator;
	}

	const SimulatorPrivate* ScriptSimulator::simulator() const
	{
		return m_simulator;
	}
