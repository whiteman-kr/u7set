...
{
	DialogWriteTuningValues::DialogWriteTuningValues(const std::vector<AppSignalParam>& params,
		...
		m_analogFormat(analogFormat)
	{
		...
		if (m_params.size() > m_defaultSignalsCount)
		{
			...
			connect(m_showAll, &QPushButton::clicked, this, &DialogWriteTuningValues::onShowAll);
		}
		...
		connect(yesBtn, &QPushButton::clicked, this, &DialogWriteTuningValues::accept);
		connect(noBtn, &QPushButton::clicked, this, &DialogWriteTuningValues::reject);
	...
	int DialogWriteTuningValues::askConfirmation(const AppSignalParam& param,
												 const TuningValue& oldValue,
												 const TuningValue& newValue,
												 E::AnalogFormat analogFormat,
												 QWidget* parent)
	{
		DialogWriteTuningValues d({param}, {oldValue}, {newValue}, analogFormat, parent);
		return d.exec();
	}

	int DialogWriteTuningValues::askConfirmation(std::vector<AppSignalParam>& params,
												 std::vector<TuningValue>& oldValues,
												 std::vector<TuningValue>& newValues,
												 E::AnalogFormat analogFormat,
												 QWidget* parent)
	{
		DialogWriteTuningValues d(std::move(params), std::move(oldValues), std::move(newValues), analogFormat, parent);
		return d.exec();
	}

	void DialogWriteTuningValues::fillTable(int maxCount)
	{
		Q_ASSERT(m_table);
		m_table->clear();
		...
		for (int row = 0; row < rowCount; row++)
		{
			...
			if (row == maxCount - 1 && row != rowCount - 1)
			{
				QTreeWidgetItem* breakItem = new QTreeWidgetItem(
					QStringList() << QString() << tr("and %1 more signals. Press \"Show all\" to view them.").arg(rowCount - row - 1));
	...
	void DialogWriteTuningValues::onShowAll()
	{
