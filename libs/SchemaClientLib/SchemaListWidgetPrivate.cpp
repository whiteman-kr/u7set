...
namespace SchemaClientLib
{
	...
	{
	public:
		SortableSchemaTreeWidgetItem(QTreeWidget* _parent,
									 const QStringList& _strings,
									 std::shared_ptr<VFrame30::SchemaDetails> _details,
									 int _type = Type) :
		...
		SortableSchemaTreeWidgetItem(QTreeWidgetItem* _parent,
									 const QStringList& _strings,
									 std::shared_ptr<VFrame30::SchemaDetails> _details,
									 int _type = Type) :
			QTreeWidgetItem(_parent, _strings, _type),
	...
	void SchemaListTreeWidget::fillList()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			{
				if (schema->m_schemaId.contains(m_filter, Qt::CaseInsensitive) == false && schema->searchForString(m_filter) == false)
				{
					// Schema filtered
					//
					filtered++;
			...
			else
			{
				...
				for (const QString& p : pathList)
				{
					...
					if (auto cpi = createdPath.find(pathString); cpi == createdPath.end())
					{
						...
						{
							treeItem =
								new SortableSchemaTreeWidgetItem{this, QStringList{} << p, {}, QTreeWidgetItem::UserType + 100000 + i};
							this->addTopLevelItem(treeItem);
						}
						else
						{
							treeItem = new SortableSchemaTreeWidgetItem{parentTreeItem,
																		QStringList{} << p,
																		{},
																		QTreeWidgetItem::UserType + 100000 + i};
						}
	...
	std::vector<QTreeWidgetItem*> SchemaListTreeWidget::searchFor(QString searchText)
	{
		...
		std::function<bool(SortableSchemaTreeWidgetItem*, const QString&)> itemHasText = [](SortableSchemaTreeWidgetItem* item,
																							const QString& searchText) -> bool
		{
			if (item->isFolder == false && item->text(0).contains(searchText, Qt::CaseInsensitive) == true)
			{
				return true;
			}
			...
			[&itemHasText, &visitTreeItems, &searchText, &result](QTreeWidgetItem* treeItem)
		{
			for (int i = 0; i < treeItem->childCount(); i++)
			{
				...
				if (item != nullptr && itemHasText(item, searchText))
				{
	...
	//
	//
	//		SchemaListWidgetPrivate
	//
	//
	SchemaListWidgetPrivate::SchemaListWidgetPrivate(std::vector<SchemaListTreeColumns> columns, bool showTags, QWidget* parent) :
		QWidget(parent)
	{
		...
		QStringList completerStringList = QSettings{}.value("SchemaListWidgetPrivate/SearchCompleter").toStringList();
		...
		if (showTags == true)
		{
			...
			connect(m_tagSelector, &UiLib::TagSelectorWidget::changed, this, &SchemaListWidgetPrivate::tagSelectorHasChanges);
		}
		...
		// --
		//
		// connect(m_simulator, &SimIdeSimulator::projectUpdated, this, &SchemaListWidgetPrivate::updateData);

		connect(m_searchEdit, &QLineEdit::returnPressed, this, &SchemaListWidgetPrivate::search);
		connect(m_filterEdit, &QLineEdit::returnPressed, this, &SchemaListWidgetPrivate::filter);
		connect(m_searchButton, &QPushButton::clicked, this, &SchemaListWidgetPrivate::search);
		connect(m_filterButton, &QPushButton::clicked, this, &SchemaListWidgetPrivate::filter);
		connect(m_resetFilterButton, &QPushButton::clicked, this, &SchemaListWidgetPrivate::resetFilter);

		connect(m_treeWidget, &SchemaListTreeWidget::openSchemaRequest, this, &SchemaListWidgetPrivate::openSchemaRequest);
		connect(m_treeWidget, &QTreeWidget::customContextMenuRequested, this, &SchemaListWidgetPrivate::treeContextMenu);
	...
	void SchemaListWidgetPrivate::setDetails(VFrame30::SchemaDetailsSet details)
	{
	...
	void SchemaListWidgetPrivate::search()
	{
		...
		if (completerStringList.contains(searchText, Qt::CaseInsensitive) == false)
		{
			...
			QSettings{}.setValue("SchemaListWidgetPrivate/SearchCompleter", completerStringList);
	...
	void SchemaListWidgetPrivate::filter()
	{
		// Search for text in schemas
		//
		assert(m_treeWidget);
	...
	void SchemaListWidgetPrivate::resetFilter()
	{
		assert(m_treeWidget);
	...
	void SchemaListWidgetPrivate::tagSelectorHasChanges()
	{
		if (m_tagSelector == nullptr)
		{
			return;
	...
	void SchemaListWidgetPrivate::treeContextMenu(const QPoint& pos)
	{
		SortableSchemaTreeWidgetItem* currentItem = dynamic_cast<SortableSchemaTreeWidgetItem*>(m_treeWidget->currentItem());
		if (currentItem == nullptr || currentItem->isFolder == true || currentItem->details == nullptr)
		{
			return;
		}
