...
namespace
{
	...
	public:
		...
		bool operator()(int index1, int index2) { return sortFunction(index1, index2, this); }
	...
	bool SignalSearchSorter::sortFunction(int index1, int index2, const SignalSearchSorter* pThis)
	{
		...
		if (index1 < 0 || index1 >= pThis->m_appSignalParamVec->size() || index2 < 0 || index2 >= pThis->m_appSignalParamVec->size())
		{
			Q_ASSERT(false);
			return index1 < index2;
		}
...
namespace SchemaClientLib
{
	...
	DialogSignalSearch::DialogSignalSearch(QWidget* parent, IAppSignalManager* appSignalManager) :
		...
		m_model(new SignalSearchItemModel{this}),
