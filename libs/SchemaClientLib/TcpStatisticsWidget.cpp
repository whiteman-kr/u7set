...
namespace SchemaClientLib
{
	...
	// TcpStatisticsWidget
	//
	TcpStatisticsWidget::TcpStatisticsWidget(bool showClose, QWidget* parent) :
		QWidget{parent}
	{
		...
		connect(reconnectButton, &QPushButton::clicked, this, &TcpStatisticsWidget::reconnectAll);
		...
		if (showClose == true)
		{
			...
			connect(closeButton, &QPushButton::clicked, this, &TcpStatisticsWidget::closeClicked);
		...
		connect(m_treeWidget, &QTreeWidget::customContextMenuRequested, this, &TcpStatisticsWidget::prepareContextMenu);
	...
	void TcpStatisticsWidget::timerEvent(QTimerEvent* event)
	{
	...
	void TcpStatisticsWidget::prepareContextMenu(const QPoint& pos)
	{
		Q_UNUSED(pos);
		...
		auto f = [this]() -> void
		{
			...
			auto mbResult = QMessageBox::warning(
				this,
				qAppName(),
				tr("Are you sure you want to reconnect the connection '%1'?\n\nData will not be available at the time of reconnection.")
					.arg(item->text(0)),
				QMessageBox::Yes,
				QMessageBox::No);
	...
	void TcpStatisticsWidget::reconnectAll()
	{
		auto mbResult = QMessageBox::warning(
			this,
			qAppName(),
			tr("Are you sure you want to reconnect all connections?\n\nData will not be available at the time of reconnection."),
			QMessageBox::Yes,
			QMessageBox::No);
		if (mbResult == QMessageBox::No)
		{
	...
	void TcpStatisticsWidget::update()
	{
		std::vector<TcpClientStatistics::Statistics> stats = TcpClientStatistics::statistics();
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (stat.state.isConnected == true)
			{
				...
				item->setText(static_cast<int>(Columns::StartTime), DateTimeToString::dateTime(startTime, false /*with no ms*/));
				...
				item->setText(static_cast<int>(Columns::UpTime),
							  QString("%1d %2:%3:%4").arg(upTime).arg(h).arg(m, 2, 10, QChar('0')).arg(s, 2, 10, QChar('0')));
