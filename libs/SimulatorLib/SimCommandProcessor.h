...
namespace Sim
{
	...
	public:
		...
		//
		bool parseFunc(QString parseFunc, EmulatorCommand* command);

		virtual void beforeAppLogicParse();
		virtual void afterAppLogicParse(std::vector<EmulatorCommand>* commands);
		...
		//
		virtual bool runCommand(const EmulatorCommand& command);
	...
	protected:
		...
		QString strAfbInst(const EmulatorCommand* command) const;
		QString strAfbInstPin(const EmulatorCommand* command) const;
		
		QString strAcc() const;
