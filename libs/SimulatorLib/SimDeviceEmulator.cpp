...
namespace Sim
{
	EmulatorCommand::EmulatorCommand(const LmCommand& command) :
		m_command(command)
	{
	}

	void EmulatorCommand::dump() const
	{
		...
		qDebug() << "EmulatorCommand: " << m_string;
		qDebug() << "\t" << str0;
		qDebug() << "\t" << str1;
		qDebug() << "\t" << str2;
		qDebug() << "\t" << str3;
	...
	QString EmulatorCommand::caption() const
	{
	...
	// DeviceEmulator
	//
	DeviceEmulator::DeviceEmulator(SimulatorPrivate* simulator) :
									 ...
									 const LogicModulesInfo& logicModulesExtraInfo)
	{
		...
		// Init RAM
		// Command processor needs initialized memory areas, so it can cache them to EmulatorCommand
		//
		initMemory();
	...
	EmulatorCommand* DeviceEmulator::command(int index)
	{
...
//		}
	...
	bool DeviceEmulator::parseCommand(const LmCommand& command, int programCounter)
	{
		...
		// --
		//
		EmulatorCommand& deviceCommand = m_commands.emplace_back(command);
	...
	bool DeviceEmulator::processOperate(std::chrono::microseconds currentTime, const QDateTime& currentDateTime, qint64 /*workcycle*/)
	{
			  ...
			  (m_logicUnit.phase == CyclePhase::IdrPhase || m_logicUnit.phase == CyclePhase::AlpPhase))
		{
			...
			EmulatorCommand& command = m_commands[commandIndex];
	...
	bool DeviceEmulator::runCommand(EmulatorCommand& deviceCommand)
	{
		// Run command
		//
		try
	...
	bool DeviceEmulator::receiveConnectionsData(std::chrono::microseconds currentTime)
	{
		...
		for (ConnectionImplPtr& c : m_connections)
		{
			//if (c->enabled() == false)
			//{
...
//								E::LogicModuleRamAccess::Read);
	...
	std::vector<DeviceCommand> DeviceEmulator::commands() const
	{
		...
		result.reserve(m_cachedCommands.size());

		for (const EmulatorCommand& c : m_cachedCommands)
		{
			result.push_back(c);	// Slicing is ok here!
		}
