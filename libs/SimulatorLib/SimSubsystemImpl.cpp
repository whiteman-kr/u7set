...
namespace Sim
{
	...
	// Subsystem
	//
	SubsystemImpl::SubsystemImpl(QString subsystemId, SimulatorPrivate* simulator) :
	...
	bool SubsystemImpl::load(const Hardware::ModuleFirmware& firmware,
							 ...
							 const LogicModulesInfo& logicModulesExtraInfo)
	{
		...
		{
			std::shared_ptr<LogicModuleImpl> logicModule = addDevice(lmInfo);
	...
	std::vector<Hardware::LogicModuleInfo> SubsystemImpl::logicModulesInfo() const
	{
		...
		{
			std::shared_ptr<LogicModuleImpl> device = dev.second;
	 ...
	 std::shared_ptr<LogicModuleImpl> SubsystemImpl::addDevice(const Hardware::LogicModuleInfo& lm)
	 {
		std::shared_ptr<LogicModuleImpl> result = std::make_shared<LogicModuleImpl>(m_simulator);
			...
			p.second == false)
		{
			...
			return std::shared_ptr<LogicModuleImpl>();
		}
	 ...
	 bool SubsystemImpl::removeDevice(QString equipmentId)
	 {
		...
		std::shared_ptr<LogicModuleImpl> device = it->second;
	 ...
	 QString SubsystemImpl::subsystemId() const
	 {
		 return m_subsystemId;
	 }

	 std::vector<std::shared_ptr<LogicModuleImpl>> SubsystemImpl::logicModules()
	 {
	 ...
	 std::shared_ptr<LogicModuleImpl> SubsystemImpl::logicModule(QString equipmentId)
	 {
		 auto it = m_devicesByEquipmentId.find(equipmentId);
