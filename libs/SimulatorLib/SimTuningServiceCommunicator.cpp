...
namespace Sim
{
	...
	TuningServiceCommunicator::TuningServiceCommunicator(SimulatorPrivate* simulator,
		...
		m_log(m_simulator->log(), "TuningCommunicator")
	{
		...
		connect(m_simulator, &SimulatorPrivate::projectUpdated, this, &TuningServiceCommunicator::projectUpdated);
	...
	SimulatorPrivate* TuningServiceCommunicator::simulator() const
	{
		return m_simulator;
	}
	...
	void TuningRequestsProcessingThread::initTuningSourcesHandlers(const TuningServiceSettings& settings)
	{
		...
		{
			std::shared_ptr<LogicModuleImpl> lm = m_sim.logicModule(ts.lmEquipmentID);
	...
	{
		std::shared_ptr<LogicModuleImpl> lm = m_tsCommunicator.simulator()->logicModule(m_lmEquipmentID);
