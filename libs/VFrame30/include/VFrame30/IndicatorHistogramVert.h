...
namespace VFrame30
{
	...
	*/
	...
	private:
		...
		void drawGrids(const std::vector<DrawGridStruct>& grids,
		...
		std::optional<AppSignalParam> getSignalParam(const Context& context, const QString& appSignalId) const;
		bool getSignalState(const Context& context,
		...
		//
		std::optional<double> getSignalValue(const Context& context, const QString& appSignalId) const;

		// Setpoints drawing
		//
		std::vector<IndicatorSetpoint> comparators(CDrawParam* drawParam, const Context* context, const QString& appSignalId) const;
		...
		{
			DrawSetpointStruct(int signalIndex_, const QRectF& barRect_, IndicatorSetpoint indicatorSetpoint_) :
		...
		void drawSetpointItems(CDrawParam* drawParam,
	...
	private:
		...
		bool m_drawGridValueForAllBars = false;  // Draw values for grid for all bars (true) or just for the first one (false) (only if
												 // drawGrid == true && drawGridValues == true)
