...
namespace SimUi
{
	...
	{
		// Save current state of ArminKey, TuningKey. It is convenient feature, so user must not switch on/off tuning options on number of
		// LMs again.
		//
		...
		//
		for (auto lms = m_simulator->logicModules(); auto& lm : lms)
		{
			auto it = std::find_if(oldLogicModules.begin(),
								   oldLogicModules.end(),
								   [&lm](const auto& oldLm)
								   {
