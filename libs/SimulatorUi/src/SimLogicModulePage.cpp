...
namespace SimUi
{
		...
		m_appSignalController(appSignalController)
	{
		...
		m_equipmentIdLabel = new QLabel{tr("EquipmentID:"), this};
		m_equipmentIdValue = new QLabel{this};

		m_subsystemIdLabel = new QLabel{tr("SubsystemID:"), this};
		m_subsystemIdValue = new QLabel{this};

		m_channelLabel = new QLabel{tr("Channel:"), this};
		m_channelValue = new QLabel{this};

		m_moduleLabel = new QLabel{tr("Module:"), this};
		m_moduleValue = new QLabel{this};

		m_stateLine = new QFrame{this};

		m_disableButton = new QPushButton{tr("Disable"), this};
		m_stateLabel = new QLabel{this};

		m_runtimeModeLabel = new QLabel{tr("Runtime Mode:"), this};
		m_runtimeModeValue = new QLabel{tr("{}"), this};

		m_sorLine = new QFrame{this};

		m_sorResetSwitchButton = new QPushButton{tr("Reset SOR"), this};
		m_sorIsSetLabel = new QLabel{tr("SOR is Set: {0}"), this};

		m_sorSetSwitch1Button = new QPushButton{tr("SOR Switch 1"), this};
		m_sorSetSwitch2Button = new QPushButton{tr("SOR Switch 2"), this};
		m_sorSetSwitch3Button = new QPushButton{tr("SOR Switch 3"), this};

		m_tuningLine = new QFrame{this};

		m_tuningModeLabel = new QLabel{tr("Tuning Mode:"), this};
		m_tuningModeValue = new QLabel{tr("{No}"), this};

		m_armingKeyButton = new QPushButton{tr("Arming Key"), this};
		m_armingKeyStateLabel = new QLabel{tr("{0}"), this};

		m_tuningKeyButton = new QPushButton{tr("Tuning Key"), this};
		m_tuningKeyStateLabel = new QLabel{tr("{0}"), this};

		m_signalsButton = new QPushButton{tr("Signals"), this};
		m_memoryButton = new QPushButton{tr("Memory Dump"), this};
		m_codeButton = new QPushButton{tr("Code"), this};

		m_schemasLabel = new QLabel{tr("Schemas:"), this};
		m_schemasList = new QTreeWidget{this};

		m_schemaFilterEdit = new QLineEdit{this};
		m_completer = new QCompleter{QStringList{}, this};
		...
		if (QByteArray ba = s.value("Simulator/SimLogicModulePage/m_schemasList").toByteArray(); ba.isEmpty() == false)
		{
			m_schemasList->header()->restoreState(ba);
		}

		if (QByteArray ba = s.value("Simulator/SimLogicModulePage/m_splitter").toByteArray(); ba.isEmpty() == false)
		{
	...
	void SimLogicModulePage::updateLogicModuleInfoInfo()
	{
		...
		m_subsystemIdValue->setText(QString("%1 (LMNumber: %2)").arg(lmInfoExtras.subsystemID).arg(lmInfoExtras.lmNumber));

		m_channelValue->setText(lmInfoExtras.subsystemChannel);

		m_moduleValue->setText(QString("%1 (%2)").arg(lmInfoExtras.caption).arg(lmInfoExtras.lmDescriptionFile));
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true && lm->isPowerOff() != toPowerOff)
		{
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true && lm->armingKey() != value)
		{
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true && lm->tuningKey() != value)
		{
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true)
		{
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true && lm->sorSetSwitch1() != value)
		{
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true && lm->sorSetSwitch2() != value)
		{
	...
	{
		if (auto lm = logicModule(); lm.has_value() == true && lm->sorSetSwitch3() != value)
		{
	...
	void SimLogicModulePage::signalsButtonClicked()
	{
		...
		SimWidgetPrivate* simWidget = nullptr;

		QWidget* pw = parentWidget();
		while (pw != nullptr)
		{
			if (dynamic_cast<SimWidgetPrivate*>(pw) != nullptr)
			{
				simWidget = dynamic_cast<SimWidgetPrivate*>(pw);
				break;
			}
			pw = pw->parentWidget();
		}
