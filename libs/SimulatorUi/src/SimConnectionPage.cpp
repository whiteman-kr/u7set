...
namespace SimUi
{
		...
		m_connectionId(connectionId)
	{
		...
		m_connectionIdLabel = new QLabel{this};
		m_disableButton = new QPushButton{tr("Disable"), this};
		m_stateLabel = new QLabel{this};

		m_connectionType = new QLabel{this};

		m_port1Label = new QLabel{this};
		m_port1TxSignals = new QPushButton{this};
		m_port1RxSignals = new QPushButton{this};
		m_port1TxBuffer = new QPushButton{this};
		m_port1RxBuffer = new QPushButton{this};

		m_port2Label = new QLabel{this};
		m_port2TxSignals = new QPushButton{this};
		m_port2RxSignals = new QPushButton{this};
		m_port2TxBuffer = new QPushButton{this};
		m_port2RxBuffer = new QPushButton{this};
		...
		connect(m_port1TxSignals,
				&QPushButton::clicked,
				[this]()
				{
					this->showTxSignals(0);
				});
		connect(m_port1RxSignals,
				&QPushButton::clicked,
				[this]()
				{
					this->showRxSignals(0);
				});
		connect(m_port2TxSignals,
				&QPushButton::clicked,
				[this]()
				{
					this->showTxSignals(1);
				});
		connect(m_port2RxSignals,
				&QPushButton::clicked,
				[this]()
				{
					this->showRxSignals(1);
				});

		connect(m_port1TxBuffer,
				&QPushButton::clicked,
				[this]()
				{
					this->showTxBuffer(0);
				});
		connect(m_port1RxBuffer,
				&QPushButton::clicked,
				[this]()
				{
					this->showRxBuffer(0);
				});
		connect(m_port2TxBuffer,
				&QPushButton::clicked,
				[this]()
				{
					this->showTxBuffer(1);
				});
		connect(m_port2RxBuffer,
				&QPushButton::clicked,
				[this]()
				{
					this->showRxBuffer(1);
	...
	void SimConnectionPage::updateData()
	{
		...
		m_connectionIdLabel->setText(tr("ConnectionID: <b>%1</b>").arg(m_connectionId));
		...
		else
		{
			m_connectionType->setText(tr("\tType:\t\t%1\n"
										 ...
										 "\tControlDataID:\t%5\n")
										  ...
										  .arg(m_connectionInfo.linkID, 4, 16, QChar('0'))
										  .arg(m_connectionInfo.linkID)
			...
			auto formPortInfo = [](const ::ConnectionPortInfo& portInfo) -> QString
			{
				QString result = QString("\nPort%1:\t%2\n\n"
									 ...
									 .arg(portInfo.lmID)
									 .arg(portInfo.txBufferAbsAddr, 4, 16, QChar('0'))
									 .arg(portInfo.txBufferAbsAddr)
									 .arg(portInfo.txDataSizeW, 4, 16, QChar('0'))
									 .arg(portInfo.txDataSizeW)
									 .arg(portInfo.txDataID, 8, 16, QChar('0'))
									 .arg(portInfo.rxBufferAbsAddr, 4, 16, QChar('0'))
									 .arg(portInfo.rxBufferAbsAddr)
									 .arg(portInfo.rxDataSizeW, 4, 16, QChar('0'))
			...
			if (ports.size() >= 1)
			{
				...
				m_port1TxSignals->setText(tr("Port%1 TX Signals (%2)").arg(ports[0].portNo).arg(ports[0].txSignals.size()));

				m_port1RxSignals->setText(tr("Port%1 RX Signals (%2)").arg(ports[0].portNo).arg(ports[0].rxSignals.size()));
			...
			if (ports.size() >= 2)
			{
				...
				m_port2TxSignals->setText(tr("Port%1 TX Signals (%2)").arg(ports[1].portNo).arg(ports[1].txSignals.size()));

				m_port2RxSignals->setText(tr("Port%1 RX Signals (%2)").arg(ports[1].portNo).arg(ports[1].rxSignals.size()));
										 ...
										 QString trx)
	{
		...
		{
			if (parent->metaObject()->className() == QString("SimulatorTabPage"))
			{
				break;
			}
