...
namespace SimUi
{
		...
		m_simulator(simulator)
	{
		...
		layout->addWidget(m_buildLabel,
						  0,
						  0,
						  Qt::AlignmentFlag::AlignLeft | Qt::AlignmentFlag::AlignLeft | Qt::AlignmentFlag::AlignVCenter);
		layout->addWidget(m_buildNoLabel,
						  0,
						  1,
						  Qt::AlignmentFlag::AlignRight | Qt::AlignmentFlag::AlignLeft | Qt::AlignmentFlag::AlignVCenter);
	...
	{
		std::function<void(QTreeWidgetItem*)> visitTreeItems = [&visitTreeItems, &state, this](QTreeWidgetItem* treeItem)
		{
			for (int i = 0; i < treeItem->childCount(); i++)
			{
				QTreeWidgetItem* item = treeItem->child(i);

				if (BaseTreeItem* baseTreeItem = dynamic_cast<BaseTreeItem*>(item); baseTreeItem != nullptr)
				{
		...
		std::function<void(QTreeWidget*)> visitTopTreeItems = [&visitTreeItems, &state, this](QTreeWidget* treeWidget)
		{
	...
	{
		BaseTreeItem::BaseTreeItem(QTreeWidgetItem* parent, const QStringList& strings) :
		...
		ConnectionTreeItem::ConnectionTreeItem(QTreeWidgetItem* parent, const Sim::Connection& connection) :
			BaseTreeItem(parent, QStringList{} << connection.connectionId() << connection.typeStr()),
		...
		void ConnectionTreeItem::contextMenu(SimProjectWidget* simProjectWidget, QPoint globalMousePos)
		{
			...
			QAction* disableAction =
				menu.addAction(QObject::tr("Disable"),
		...
		AppLogicSchemasTreeItem::AppLogicSchemasTreeItem(QTreeWidgetItem* parent) :
			BaseTreeItem(parent, QStringList{} << "Application Logic Schemas")
		{
			QFont f = font(0);
