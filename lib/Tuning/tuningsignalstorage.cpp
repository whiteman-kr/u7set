...
bool TuningSignalStorage::loadSignals(const QByteArray& data, QString *errorCode)
{
	...
	while (!reader.atEnd())
	{
		...
		{
			std::shared_ptr<Signal> object = std::make_shared<Signal>();
			...
			if (reader.attributes().hasAttribute("Type"))
			{
				...
				object->setSignalType(t == "A" ? E::SignalType::Analog : E::SignalType::Discrete);
			}
...
Signal* TuningSignalStorage::signalPtrByIndex(int index) const
{
...
Signal* TuningSignalStorage::signalPtrByHash(Hash hash) const
{
