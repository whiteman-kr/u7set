...
void TuningSignalManager::requestReadTuningSignals()
{
    ...
    {
		const Signal* object = m_signals.signalPtrByIndex(m_readTuningSignalIndex + i);
        if (object == nullptr)
        {
            assert(object);
            continue;
...
void TuningSignalManager::slot_analog(QString appSignalID, bool* result, bool* ok)
{
	...
	Signal* signal = m_signals.signalPtrByHash(hash);
	if (signal == nullptr)
	{
		*ok = false;
		return;
...
void TuningSignalManager::slot_highLimit(QString appSignalID, float *result, bool* ok)
{
	...
	Signal* object = m_signals.signalPtrByHash(hash);
	if (object == nullptr)
	{
		*ok = false;
		return;
...
void TuningSignalManager::slot_getParam(QString appSignalID, Signal& result, bool* ok)
{
	Hash hash = ::calcHash(appSignalID);
