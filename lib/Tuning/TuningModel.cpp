...
bool TuningItemSorter::sortFunction(const TuningModelItem &o1, const TuningModelItem &o2, int column, Qt::SortOrder order) const
{
	...
	case TuningItemModel::Columns::CustomAppSignalID:
		{
			v1 = o1.param.customAppSignalID();
			v2 = o2.param.customAppSignalID();
		}
		break;
	...
	if (v1 == v2)
	{
		return o1.param.customAppSignalID() < o2.param.customAppSignalID();
	}
...
Signal* TuningItemModel::param(int index)
{
...
QVariant TuningItemModel::data(const QModelIndex &index, int role) const
{
	...
	if (role == Qt::DisplayRole)
	{
		...
		{
			return item.param.customAppSignalID();
		}
		...
		{
			return QString::number(item.param.unitID());
		}
        ...
        if (displayIndex == static_cast<int>(Columns::Value))
		{
			if (item.state.valid() == true)
			{
				...
				{
					QString valueString = QString::number(item.state.value(), 'f', item.param.decimalPlaces());

					if (item.state.userModified() == true)
                    {
						QString editValueString = QString::number(item.state.editValue(), 'f', item.param.decimalPlaces());
			...
			if (item.param.isAnalog())
            {
				if (item.limitsUnbalance())
                {
					QString str = tr("Base %1, read %2")
							.arg(QString::number(item.param.lowEngeneeringUnits(), 'f', item.param.decimalPlaces()))
							.arg(QString::number(item.state.readLowLimit(), 'f', item.param.decimalPlaces()));
					return str;
                }
                else
                {
					return QString::number(item.param.lowEngeneeringUnits(), 'f', item.param.decimalPlaces());
                }
            }
            else
            {
        ...
        if (displayIndex == static_cast<int>(Columns::HighLimit))
        {
			if (item.param.isAnalog())
            {
				...
				{
					QString str = tr("Base %1, read %2")
							.arg(QString::number(item.param.highEngeneeringUnits(), 'f', item.param.decimalPlaces()))
							.arg(QString::number(item.state.readHighLimit(), 'f', item.param.decimalPlaces()));
					return str;
                }
                else
                {
					return QString::number(item.param.highEngeneeringUnits(), 'f', item.param.decimalPlaces());
                }
            }
            else
            {
        ...
        if (displayIndex == static_cast<int>(Columns::Default))
		{
			if (item.param.isAnalog())
			{
				return QString::number(item.param.tuningDefaultValue(), 'f', item.param.decimalPlaces());
			}
			else
			{
