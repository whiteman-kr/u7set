...
namespace VFrame30
{
	...
	double CVFrameUtils::RoundDisplayPoint(double value)	// По сути floor, используется для приведения координат при SchemeUnit::Display
	{
		return floor(value);
	}

	double CVFrameUtils::RoundPoint(double p, SchemeUnit unit)				// Округлить значение в зависимости от типа региональной метрики 
	{
		switch (unit)
		{ 
