...
namespace Sim
{
	...
	bool FlagsReadStruct::create(const AppSignal& s, const std::unordered_map<Hash, AppSignal>& signalParams, ScopedLog& log)
	{
		...
		for (const auto& [flagType, flagAppSignalId] : stateFlagsSignals)
		{
			...
			if (auto flagSignalIt = signalParams.find(flagSignalHash); flagSignalIt == signalParams.end())
			{
				// Error, flag signal is not found
				//
				log.writeError(
					QObject::tr("AppSignalManager::signalFlags(%1) cannot find flag signal %2").arg(s.appSignalID()).arg(flagAppSignalId));
	...
	AppSignalStateFlags FlagsReadStruct::signalFlags(const Ram& ram, ScopedLog& log) const
	{
		...
		for (size_t i = 0; i < flagCount; i++)
		{
			...
			if (bool readOk =
					ram.readBit(flagAddress.offset(), static_cast<quint16>(flagAddress.bit()), &flagValue, E::ByteOrder::BigEndian);
				readOk == false)
			{
				log.writeError(
					QObject::tr("AppSignalManager::signalFlags error read flag signal by address %1").arg(flagAddress.toString()));
	...
	//
	//
	AppSignalManagerImpl::AppSignalManagerImpl(SimulatorPrivate* simulator, QObject* /*parent*/) :
		m_simulator(simulator),
		m_log(simulator->log(), "AppSignalManagerImpl")
	{
		assert(m_simulator);
	...
	AppSignalManagerImpl::~AppSignalManagerImpl()
	{
		resetAll();
	}

	QString AppSignalManagerImpl::ramDump(QString logicModuleId) const
	{
	...
	void AppSignalManagerImpl::resetAll()
	{
		resetSignalParam();
		resetRam();
		return;
	...
	void AppSignalManagerImpl::resetSignalParam()
	{
		{
	...
	void AppSignalManagerImpl::resetRam()
	{
		QWriteLocker wl(&m_ramLock);
	...
	bool AppSignalManagerImpl::load(QString fileName)
	{
		resetAll();
									   ...
									   TimeStamp systemTime)
	{
...

		{
			...
			m_ramTimes[lmHash] = tm; // keep record in m_ramTimes even for for null
									 // ram, as it is useful for getting nonvalid point for trends
		}
	...
	std::shared_ptr<TrendLib::RealtimeData> AppSignalManagerImpl::trendData(const QString& trendId,
	...
	std::optional<AppSignal> AppSignalManagerImpl::signalParamExt(const QString& appSignalId) const
	{
		return signalParamExt(::calcHash(appSignalId));
	}

	std::optional<AppSignal> AppSignalManagerImpl::signalParamExt(Hash hash) const
	{
	...
	Hash AppSignalManagerImpl::customToAppSignal(Hash customSignalHash) const
	{
		QReadLocker locker(&m_signalParamLock);
	...
	AppSignalState AppSignalManagerImpl::signalState(Hash signalHash, bool* found, bool applyOverride) const
	{
		...
		//
		{
			...
			// Set flags
			//
			if (auto flagIt = m_flagsStruct.find(signalHash); flagIt != m_flagsStruct.end())
			{
			...
			case E::SignalType::Discrete:
				{
					quint16 data = 0;
					bool ok = ram.readBit(actualAddress.offset(),
										  static_cast<quint16>(actualAddress.bit()),
										  &data,
										  byteOrder,
										  ramAccess,
										  applyOverride);
	...
	bool AppSignalManagerImpl::getUpdateForRam(const QString& equipmentId, Sim::Ram* ram) const
	{
		Q_ASSERT(ram);
	...
	std::vector<Hash> AppSignalManagerImpl::signalHashes() const
	{
	...
	std::vector<AppSignalParam> AppSignalManagerImpl::signalList() const
	{
	...
	bool AppSignalManagerImpl::signalExists(Hash hash) const
	{
		QReadLocker rl(&m_signalParamLock);
	...
	E::SignalType AppSignalManagerImpl::signalType(Hash signalHash, bool* found) const
	{
		Q_UNUSED(signalHash);
		Q_UNUSED(found);
		Q_ASSERT(false); // TO DO
	...
	const SimulatorPrivate* AppSignalManagerImpl::simulator() const
	{
		return m_simulator;
	}

	SimulatorPrivate* AppSignalManagerImpl::simulator()
	{
		return m_simulator;
	}
