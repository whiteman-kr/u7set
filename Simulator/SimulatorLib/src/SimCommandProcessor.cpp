...
namespace Sim
{
	...
	bool CommandProcessor::parseFunc(QString parseFunc, EmulatorCommand* command)
	{
		if (command == nullptr)
		{
			assert(command);
	...
	void CommandProcessor::afterAppLogicParse(std::vector<EmulatorCommand>* /*commands*/)
	{
	...
	bool CommandProcessor::runCommand([[maybe_unused]]const EmulatorCommand& command)
	{
		// Must be implemented in derived class
		//
		assert(false);
	...
	QString CommandProcessor::strAfbInst(const EmulatorCommand* command) const
	{
	...
	QString CommandProcessor::strAfbInstPin(const EmulatorCommand* command) const
	{
		AfbComponent afb = m_device->afbComponent(command->m_afbOpCode);
		if (afb.isNull() == true)
		{
