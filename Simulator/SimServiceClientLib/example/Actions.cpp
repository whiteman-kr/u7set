...
const std::map<QString, Actions::ActionFunc> Actions::actions{
	{QStringLiteral("exit"), &Actions::exitFunc},
	{QStringLiteral("help"), &Actions::help2},
	{QStringLiteral("?"), &Actions::help2},

	{QStringLiteral("ping"), &Actions::ping},
	{QStringLiteral("status"), &Actions::status},
	{QStringLiteral("start"), &Actions::start},
	{QStringLiteral("stop"), &Actions::stop},
	{QStringLiteral("pause"), &Actions::pause},

	{QStringLiteral("get_signal_list"), &Actions::get_signal_list},
	{QStringLiteral("get_signal_param"), &Actions::get_signal_param},
	{QStringLiteral("get_signal_state"), &Actions::get_signal_state},

	{QStringLiteral("get_module_list"), &Actions::get_module_list},
	{QStringLiteral("lm_status"), &Actions::lm_status},
	{QStringLiteral("lm_set_flag"), &Actions::lm_set_flag},

	{QStringLiteral("override_signal"), &Actions::override_signal},
	{QStringLiteral("remove_override"), &Actions::remove_override},
	{QStringLiteral("get_overridden_signals"), &Actions::get_overridden_signals},

	{QStringLiteral("take_snapshot"), &Actions::take_snapshot},
	{QStringLiteral("apply_snapshot"), &Actions::apply_snapshot},
};

void Actions::exitFunc(Sim::SimServiceClient&, const QStringList&)
{
	std::exit(0);
}
...
void Actions::help()
{
	...
	std::cout << "  take_snapshot <SNAPSHOTID>          take snapshot, save to disk.\n";
	std::cout << "  apply_snapshot <SNAPSHOTID>         Load snapshot from disk and apply it.\n";
	return;
}

void Actions::help2(Sim::SimServiceClient&, const QStringList&)
{
	help();
	return;
}

void Actions::ping(Sim::SimServiceClient& client, const QStringList& args)
{
...
void Actions::status(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::start(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::pause(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::stop(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::get_signal_list(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::get_signal_param(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() <= 1)
	{
		// Get for all signals
...
void Actions::get_signal_state(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() <= 1)
	{
...
void Actions::get_module_list(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::lm_status(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() < 2)
	{
...
void Actions::lm_set_flags_help()
{
...
void Actions::lm_set_flag(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() < 4)
	{
		lm_set_flags_help();
...
void Actions::override_signal_help()
{
...
void Actions::override_signal(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() < 4)
	{
		override_signal_help();
...
void Actions::remove_override_help()
{
	std::cout << "  remove_override <#ID1> ... [#IDN]      Remove overridden signal(s).\n";
}

void Actions::remove_override(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() < 2)
	{
		remove_override_help();
...
void Actions::get_overridden_signals_help()
{
	std::cout << "  get_overridden_signals                   Get all overridden signals.\n";
}

void Actions::get_overridden_signals(Sim::SimServiceClient& client, [[maybe_unused]] const QStringList& args)
{
...
void Actions::take_snapshot(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() < 2)
	{
		std::cout << "take_snapshot error, SnapshotID is missing\n";
		std::cout << "take_snapshot <SNAPSHOTID>\n";
		return;
	}

	QString snapshotId = args[1];

	auto result = client.TakeSnapshot(snapshotId);
	if (result.has_value() == false)
	{
		std::cout << "TakeSnapshot error: " << result.error().toStdString() << "\n";
		return;
	}

	const QByteArray& snapshotData = result.value();
	QString fileName = snapshotId + ".u7snap";

	QFile file{fileName};
	if (file.open(QFile::WriteOnly) == false)
	{
		std::cout << "Cannot open file " << file.fileName().toStdString() << " for writing.\n";
		return;
	}

	auto written = file.write(snapshotData);
	if (written != snapshotData.size())
	{
		std::cout << "Write file " << file.fileName().toStdString() << " error: " << file.errorString().toStdString() << "\n";
		return;
	}

	std::cout << "Snapshot written to file: " << file.fileName().toStdString() << "\n";
	std::cout << "size: " << file.size() << "\n";
	return;
}

void Actions::apply_snapshot(Sim::SimServiceClient& client, const QStringList& args)
{
	if (args.size() < 2)
	{
		std::cout << "apply_snapshot error, SnapshotID is missing\n";
		std::cout << "apply_snapshot <SNAPSHOTID>\n";
		return;
	}
	...
	{
		std::cout << "Cannot open file " << file.fileName().toStdString() << " for reading.\n";
		return;
	}

	QByteArray snapshotData = file.readAll();

	auto result = client.ApplySnapshot(snapshotData);
	if (result.has_value() == false)
	{
		std::cout << "ApplySnapshot error: " << result.error().toStdString() << "\n";
		return;
	}
