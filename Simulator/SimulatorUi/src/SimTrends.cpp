...
namespace SimUi
{
	...
	bool SimTrends::startTrendApp(std::shared_ptr<SimIdeSimulator> simulator,
	...
	void SimTrendsWidget::signalsButton()
	{
		...
		{
		public:
			SignalHasTag(const Sim::AppSignalManager* sm) :
				signalManager(sm)
			{
			}
	...
	void SimTrendsWidget::fetchTrendData()
	{
		...
		std::shared_ptr<TrendLib::RealtimeData> data =
			m_simulator->appSignalManager().trendData(windowTitle(), signalSet().trendSignalsHashes(), &minState, &maxState);
													...
													TrendLib::TrendStateItem maxState)
	{
		...
		if (updateByTimer == true || (minTime >= TimeStamp{startTime().timeStamp - duration() / 10} &&
