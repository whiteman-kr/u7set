...
namespace SimUi
{
	...
	SimLogicModulePage* SimBasePage::logicModulePage(QString lmEquipmnetId, QWidget* parent)
	{
		...
		for (SimBasePage* page : m_pages)
		{
			...
			if (cp != nullptr && cp->equipmentId() == lmEquipmnetId && cp->property("SimParentObject") == parentValue)
			{
				return cp;
			}
		}
	...
	SimConnectionPage* SimBasePage::connectionPage(QString connectionId, QWidget* parent)
	{
		...
		for (SimBasePage* page : m_pages)
		{
			...
			if (cp != nullptr && cp->connectionId() == connectionId && cp->property("SimParentObject") == parentValue)
			{
				return cp;
			}
		}
	...
	SimSelectSchemaPage* SimBasePage::selectSchemaPage(QWidget* parent)
	{
		...
		for (SimBasePage* page : m_pages)
		{
			...
			if (cp != nullptr && cp->property("SimParentObject") == parentValue)
			{
				return cp;
			}
		}
