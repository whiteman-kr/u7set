...
namespace SimUi
{
	...
	SimWidgetPrivate* SimSchemaWidget::simWidget()
	{
		SimWidgetPrivate* result = nullptr;

		QWidget* parent = this->parentWidget();
		while (parent != nullptr)
		{
			result = dynamic_cast<SimWidgetPrivate*>(parent);
			if (result != nullptr)
			{
				break;
			}
											...
											const QList<QMenu*> customMenu)
	{
		...
		//
		if (QStringList monitors = m_simulator->software().monitors(); monitors.isEmpty() == false)
		{
			menu.addSeparator();
