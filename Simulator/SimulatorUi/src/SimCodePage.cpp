...
namespace SimUi
{
	...
	// SimCodeModel
	//
	SimCodeModel::SimCodeModel(SimIdeSimulator* simulator, QString lmEquipmentId, QObject* parent) :
	...
	QVariant SimCodeModel::headerData(int section, Qt::Orientation /*orientation*/, int role) const
	{
		if (role == Qt::DisplayRole)
		{
			...
			case CodePageColumns::Row:
				return tr("Row");
			case CodePageColumns::Address:
				return tr("Address");
			case CodePageColumns::Code:
				return tr("Code");
			default:
				assert(false);
				return QVariant();
	...
	QVariant SimCodeModel::data(const QModelIndex& index, int role /*= Qt::DisplayRole*/) const
	{
		...
		if (role == Qt::DisplayRole)
		{
			...
			{
			case CodePageColumns::Row:
				return QString("%1 ").arg(row, 6, 10, QChar(' '));
			case CodePageColumns::Address:
				return QString(" 0x%1 ").arg(command.m_offset, 6, 16, QChar('0'));
			case CodePageColumns::Code:
				return QString(" %1").arg(command.m_string);
			default:
				assert(false);
				return QVariant();
			}
	...
	// SimCodePage
	//
	SimCodePage::SimCodePage(SimIdeSimulator* simulator, QString lmEquipmentId, QWidget* parent) :
	...
	QString SimCodePage::equipmentId() const
	{
