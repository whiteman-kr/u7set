...
namespace SimUi
{
	SimOverridePane::SimOverridePane(Sim::Simulator& simulator, ISimPropertyStorage& propertyStorage, QWidget* parent) :
		QWidget{parent},
		m_simulator{simulator},
		m_propertyStorage{propertyStorage}
	{
		m_treeWidget = new QOverrideListWidget(m_simulator, m_propertyStorage, this);
			...
			for (const QString& id : signalIds)
			{
				...
				if (sp->isConst() == true)
				{
					QMessageBox::critical(this,
										  qAppName(),
										  tr("Value for signal %1 cannot be is overriden as it was optimized to const.").arg(id));
					return;
				}
			}
			...
			case Qt::Key::Key_Space:
				{
					...
					std::sort(selectedItems.begin(),
							  selectedItems.end(),
							  [this](QTreeWidgetItem* i1, QTreeWidgetItem* i2)
							  {
					...
					for (QTreeWidgetItem* selectedItem : selectedItems)
					{
						...
						if (item->m_overrideSignal.signalType() == E::SignalType::Discrete)
						{
							...
							overrideData.emplace_back(appSignalId,
													  Sim::OverrideSignalMethod::Value,
													  QVariant::fromValue<qint32>(currentValue));
						}
					}
	...
	void SimOverridePane::contextMenuEvent(QContextMenuEvent* event)
	{
		...
		// Save to files
		//
		QAction* saveWorkspaceAction = menu.addAction(tr("Save Worksapce..."),
													  [this]
													  {
														  saveWorkspace();
													  });
		...
		// Save to files
		//
		QAction* restoreWorkspaceAction = menu.addAction(tr("Restore Workspace..."),
														 [this]
														 {
															 restoreWorkspace();
														 });
		...
		QAction* copyAction = menu.addAction(tr("Copy Value"),
											 [this]()
											 {
												 ...
												 {
													 QString text =
														 selected.back()->text(static_cast<int>(QOverrideTreeWidgetItem::Columns::Value));
	...
	void SimOverridePane::updateValueColumn()
	{
		...
		for (int i = 0; i < itemCount; i++)
		{
			...
			auto it = std::find_if(currentSignals.begin(),
								   currentSignals.end(),
	...
	void SimOverridePane::removeSelectedSignals()
	{
		...
		for (QTreeWidgetItem* selectedItem : selectedItems)
		{
			...
			{
				if (auto itemBelow = dynamic_cast<QOverrideTreeWidgetItem*>(m_treeWidget->itemBelow(item)); itemBelow != nullptr)
				{
	...
	void SimOverridePane::addSignal()
	{
		...
		do
		{
			bool ok = false;
			QString signalId =
				QInputDialog::getText(this, tr("Add Signal to Override"), tr("SignalID:"), QLineEdit::Normal, defaultText, &ok).trimmed();
	...
	void SimOverridePane::showSetValueDialog(const QStringList& appSignalIds)
	{
		...
		SimOverrideUI::OverrideDialog::showDialog(overrideValueSignals, m_simulator, m_propertyStorage, this);
		SimOverrideUI::OverrideDialog::setViewOptions(appSignalIds, m_currentBase, m_currentFormat, m_currentPrecision);
	...
	QOverrideListWidget::QOverrideListWidget(Sim::Simulator& simulator, ISimPropertyStorage& propertyStorage, QWidget* parent) :
		QTreeWidget{parent},
		m_simulator{simulator},
		m_propertyStorage{propertyStorage}
	{
	...
	void QOverrideListWidget::mouseMoveEvent(QMouseEvent* event)
	{
		...
		if (m_dragAppSignalIds.isEmpty() == false && event->buttons().testFlag(Qt::LeftButton) == true &&
