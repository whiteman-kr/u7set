...
QStringList AppLogOutputWidget::m_data;

AppLogOutputWidget::AppLogOutputWidget(QWidget* parent) :
	QTextEdit(parent)
{

	if (appLogOriginalMessageHandler == nullptr)
	{
		appLogOriginalMessageHandler = qInstallMessageHandler(appLogOutputHandler);
	}
...
void AppLogOutputWidget::appLogOutputHandler(QtMsgType type, const QMessageLogContext& context, const QString& msg)
{
	if (QString(context.category) == QLatin1String("testsuite.applog"))
	{
		QString color;
		switch (type)
		{
		...
		QString time = DateTimeToString::time(QTime::currentTime(), true /*with ms*/);
	...
	{
		appLogOriginalMessageHandler(type, context, msg);
	}

	return;
}

void AppLogOutputWidget::timerEvent(QTimerEvent*)
{
	QStringList data;
	{
		QMutexLocker l(&m_mutex);
...
void AppLogOutputWidget::contextMenuEvent(QContextMenuEvent* event)
{
...
void AppLogOutputWidget::keyPressEvent(QKeyEvent* e)
{
